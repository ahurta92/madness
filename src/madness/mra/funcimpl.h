...
namespace madness {
template <typename T, std::size_t NDIM> class DerivativeBase;

template <typename T, std::size_t NDIM> class FunctionImpl;

template <typename T, std::size_t NDIM> class FunctionNode;
...
template <typename keyT> class SimplePmap : public WorldDCPmapInterface<keyT> {
private:
  const int nproc;
  const ProcessID me;
...
template <typename keyT> class LevelPmap : public WorldDCPmapInterface<keyT> {
private:
  const int nproc;
...
private:
  ...
  double _norm_tree;   ///< After norm_tree will contain norm of coefficients
                       ///< summed up tree
  bool _has_children;  ///< True if there are children
  coeffT buffer;       ///< The coefficients, if any
  double dnorm = -1.0; ///< norm of the d coefficients, also defined if there
                       ///< are no d coefficients
  double snorm = -1.0; ///< norm of the s coefficients

public:
  typedef WorldContainer<Key<NDIM>, FunctionNode<T, NDIM>>
      dcT; ///< Type of container holding the nodes
  /// Default constructor makes node without coeff or children
  FunctionNode() : _coeffs(), _norm_tree(1e300), _has_children(false) {}
  ...
  /// take ownership.
  explicit FunctionNode(const coeffT &coeff, bool has_children = false)
      : _coeffs(coeff), _norm_tree(1e300), _has_children(has_children) {}

  explicit FunctionNode(const coeffT &coeff, double norm_tree,
                        bool has_children)
      : _coeffs(coeff), _norm_tree(norm_tree), _has_children(has_children) {}

  explicit FunctionNode(const coeffT &coeff, double norm_tree, double snorm,
                        double dnorm, bool has_children)
      : _coeffs(coeff), _norm_tree(norm_tree), _has_children(has_children),
        dnorm(dnorm), snorm(snorm) {}

  FunctionNode(const FunctionNode<T, NDIM> &other) { *this = other; }
  ...
  /// Copy with possible type conversion of coefficients, copying all other
  /// state

  /// Choose to not overload copy and type conversion operators
  /// so there are no automatic type conversions.
  template <typename Q> FunctionNode<Q, NDIM> convert() const {
    return FunctionNode<Q, NDIM>(madness::convert<Q, T>(coeff()), _norm_tree,
                                 snorm, dnorm, _has_children);
  }

  /// Returns true if there are coefficients in this node
  bool has_coeff() const { return _coeffs.has_data(); }
  ...
  coeffT &coeff() {
    MADNESS_ASSERT(_coeffs.ndim() == -1 ||
                   (_coeffs.dim(0) <= 2 * MAXK && _coeffs.dim(0) >= 0));
  ...
  const coeffT &coeff() const { return const_cast<const coeffT &>(_coeffs); }
...
public:
  ...
  void reduceRank(const double &eps) { _coeffs.reduce_rank(eps); }
  ...
  void set_has_children_recursive(const typename FunctionNode<T, NDIM>::dcT &c,
                                  const Key<NDIM> &key) {
    // madness::print("   set_chi_recu: ", key, *this);
    // PROFILE_MEMBER_FUNC(FunctionNode); // Too fine grain for routine
    // profiling
    if (!(has_children() || has_coeff() || key.level() == 0)) {
      ...
      // Task on next line used to be TaskAttributes::hipri()) ... but deferring
      // execution of this makes sense since it is not urgent and lazy
      // connection will likely mean that less forwarding will happen since the
      // upper level task will have already made the connection.
      const_cast<dcT &>(c).task(
          parent, &FunctionNode<T, NDIM>::set_has_children_recursive, c,
          parent);
      // const_cast<dcT&>(c).send(parent,
      // &FunctionNode<T,NDIM>::set_has_children_recursive, c, parent);
  ...
  void set_coeff(const coeffT &coeffs) {
    coeff() = coeffs;
    if ((_coeffs.has_data()) and
        ((_coeffs.dim(0) < 0) || (_coeffs.dim(0) > 2 * MAXK))) {
      print("set_coeff: may have a problem");
      print("set_coeff: coeff.dim[0] =", coeffs.dim(0),
            ", 2* MAXK =", 2 * MAXK);
    }
  ...
  void gaxpy_inplace(const T &alpha, const FunctionNode<Q, NDIM> &other,
                     const R &beta) {
  ...
  void accumulate2(const tensorT &t,
  ...
  void print_json(std::ostream &s) const {
    ...
    if (nt == 1e300)
      nt = 0.0;
    s << norm << ",\"norm_tree\":" << nt << ",\"snorm\":" << this->get_snorm()
      << ",\"dnorm\":" << this->get_dnorm()
...
std::ostream &operator<<(std::ostream &s, const FunctionNode<T, NDIM> &node) {
  s << "(has_coeff=" << node.has_coeff()
    << ", has_children=" << node.has_children() << ", norm=";
  ...
  if (nt == 1e300)
    nt = 0.0;
  s << norm << ", norm_tree, s/dnorm =" << nt << ", " << node.get_snorm() << " "
    << node.get_dnorm() << "), rank=" << node.coeff().rank() << ")";
  if (node.coeff().is_assigned())
    s << " dim " << node.coeff().dim(0) << " ";
  return s;
}

/// returns true if the result of a hartree_product is a leaf node (compute norm
/// & error)
template <typename T, size_t NDIM> struct hartree_leaf_op {
  ...
  bool operator()(const Key<NDIM> &key, const Tensor<T> &fcoeff,
                  const Tensor<T> &gcoeff) const {
    ...
    const double thresh =
        f->truncate_tol(tol, key) * 0.3; // custom factor to "ensure" accuracy
    ...
    //            const double error=sqrt(fnorm*fnorm*gnorm*gnorm -
    //            sfnorm*sfnorm*sgnorm*sgnorm);

    if (error < thresh)
      return true;
    return false;
  }
  template <typename Archive> void serialize(Archive &ar) { ar & f & k; }
};

/// returns true if the result of the convolution operator op with some provided
/// coefficients will be small
template <typename T, size_t NDIM, typename opT> struct op_leaf_op {
  typedef FunctionImpl<T, NDIM> implT;
  ...
  /// pre-determination: we can't know if this will be a leaf node before we got
  /// the final coeffs
  bool operator()(const Key<NDIM> &key) const { return true; }

  /// post-determination: return true if operator and coefficient norms are
  /// small
  bool operator()(const Key<NDIM> &key, const GenTensor<T> &coeff) const {
    if (key.level() < 2)
      return false;
  ...
  template <typename Archive> void serialize(Archive &ar) { ar & op & f; }
};
...
struct hartree_convolute_leaf_op {
  ...
  bool operator()(const Key<NDIM> &key, const Tensor<T> &fcoeff,
                  const Tensor<T> &gcoeff) const {
    //        bool operator()(const Key<NDIM>& key, const GenTensor<T>& coeff)
    //        const {
    ...
    // now check if the norm of this and the norm of the operator are
    // significant
    const std::vector<Key<NDIM>> &disp = op->get_disp(key.level());
    const Key<NDIM> &d =
        *disp.begin(); // use the zero-displacement for screening
    const double opnorm = op->norm(key.level(), d, key);
    const double final_norm = opnorm * sfnorm * sgnorm;
    if (final_norm < thresh)
      return true;

    return false;
  }
  template <typename Archive> void serialize(Archive &ar) { ar & f & op; }
};

template <typename T, size_t NDIM> struct noop {
  void operator()(const Key<NDIM> &key, const GenTensor<T> &coeff,
                  const bool &is_leaf) const {}
  bool operator()(const Key<NDIM> &key, const GenTensor<T> &fcoeff,
                  const GenTensor<T> &gcoeff) const {
...
template <typename T, std::size_t NDIM> struct insert_op {
  typedef FunctionImpl<T, NDIM> implT;
  ...
  void operator()(const keyT &key, const coeffT &coeff,
                  const bool &is_leaf) const {
    MADNESS_ASSERT(impl->get_coeffs().is_local(key));
    impl->get_coeffs().replace(key, nodeT(coeff, not is_leaf));
  }
  template <typename Archive> void serialize(Archive &ar) { ar & impl; }
};

/// inserts/accumulates coefficients into impl's tree

/// NOTE: will use buffer and will need consolidation after operation ended !!
/// NOTE !!
template <typename T, std::size_t NDIM> struct accumulate_op {
  typedef GenTensor<T> coeffT;
  typedef FunctionNode<T, NDIM> nodeT;
  ...
  void operator()(const Key<NDIM> &key, const coeffT &coeff,
                  const bool &is_leaf) const {
    if (coeff.has_data())
      impl->get_coeffs().task(key, &nodeT::accumulate, coeff,
                              impl->get_coeffs(), key, impl->get_tensor_args());
  }
  template <typename Archive> void serialize(Archive &ar) { ar & impl; }
};

template <size_t NDIM> struct true_op {
...
template <typename T, std::size_t NDIM> struct ShallowNode {
  typedef GenTensor<T> coeffT;
  coeffT _coeffs;
  bool _has_children;
  double dnorm = -1.0;
...
/// E.g. if a 6D function is composed of two 3D functions their coefficients
/// must be tracked. We might need coeffs from a box that does not exist, and to
/// avoid searching for parents we track which are their required respective
/// boxes.
///  - CoeffTracker will refer either to a requested key, if it exists, or to
///  its
///    outermost parent.
///  - Children must be made in sequential order to be able to track correctly.
///
/// Usage: 	1. make the child of a given CoeffTracker.
///			   If the parent CoeffTracker refers to a leaf node
///(flag is_leaf)
///            the child will refer to the same node. Otherwise it will refer
///            to the child node.
///			2. retrieve its coefficients (possible communication/
/// returns a Future).
...
template <typename T, size_t NDIM> class CoeffTracker {
...
public:
  ...
  CoeffTracker(const CoeffTracker &other)
      : impl(other.impl), key_(other.key_), is_leaf_(other.is_leaf_),
        coeff_(other.coeff_), dnorm_(other.dnorm_) {};
  ...
  CoeffTracker make_child(const keyT &child) const {
    ...
    // fast return
    if ((not impl) or impl->is_on_demand())
      return CoeffTracker(*this);
    ...
    if (impl) {
      ...
      if (is_leaf_ == no) {
        result.key_ = child;
        // check if child is direct descendent of this, but root node is special
        // case
        if (child.level() > 0)
          MADNESS_ASSERT(result.key().level() == key().level() + 1);
      }
  ...
  Future<CoeffTracker> activate() const {
    ...
    // fast return
    if (not impl)
      return Future<CoeffTracker>(CoeffTracker());
    if (impl->is_on_demand())
      return Future<CoeffTracker>(CoeffTracker(impl));

    // this will return a <keyT,nodeT> from a remote node
    ProcessID p = impl->get_coeffs().owner(key());
    Future<datumT> datum1 =
        impl->task(p, &implT::find_datum, key_, TaskAttributes::hipri());
...
private:
  ...
  CoeffTracker forward_ctor(const CoeffTracker &other,
                            const datumT &datum) const {
...
/// abused ... NOTHING except FunctionImpl methods should mess with
/// FunctionImplData. The LB stuff might have to be an exception.
template <typename T, std::size_t NDIM>
class FunctionImpl : public WorldObject<FunctionImpl<T, NDIM>> {
private:
  typedef WorldObject<FunctionImpl<T, NDIM>>
      woT; ///< Base class world object type
public:
  typedef T typeT;
  typedef FunctionImpl<T, NDIM> implT; ///< Type of this class (implementation)
  typedef std::shared_ptr<FunctionImpl<T, NDIM>>
      pimplT;                ///< pointer to this class
  typedef Tensor<T> tensorT; ///< Type of tensor for anything but to hold coeffs
  typedef Vector<Translation, NDIM>
      tranT;                           ///< Type of array holding translation
  ...
  typedef WorldContainer<keyT, nodeT>
      dcT; ///< Type of container holding the coefficients
...
private:
  ...
  std::vector<Vector<double, NDIM>>
      special_points;   ///< special points for further refinement (needed for
                        ///< composite functions or multiplication)
...
public:
  ...
  FunctionImpl(const FunctionFactory<T, NDIM> &factory)
      : WorldObject<implT>(factory._world), world(factory._world),
        k(factory._k), thresh(factory._thresh),
        ...
        truncate_mode(factory._truncate_mode), autorefine(factory._autorefine),
        ...
        //		  , redundant(false)
        ,
        tree_state(factory._tree_state), coeffs(world, factory._pmap, false)
  //, bc(factory._bc)
  {
    ...
    if (empty) {          // Do not set any coefficients at all
      ...
      // set the union of the special points of functor and the ones explicitly
      // given to FunctionFactory
      std::vector<coordT> functor_special_points = functor->special_points();
      if (!functor_special_points.empty())
        special_points.insert(special_points.end(),
                              functor_special_points.begin(),
                              functor_special_points.end());

      typename dcT::const_iterator end = coeffs.end();
      for (typename dcT::const_iterator it = coeffs.begin(); it != end; ++it) {
        if (it->second.is_leaf())
          woT::task(coeffs.owner(it->first), &implT::project_refine_op,
                    it->first, do_refine, special_points);
      }
    ...
    if (factory._fence && (functor || !empty))
      world.gop.fence();
  }
  ...
  FunctionImpl(const FunctionImpl<Q, NDIM> &other,
               ...
               bool dozero)
      : WorldObject<implT>(other.world), world(other.world), k(other.k),
        thresh(other.thresh), initial_level(other.initial_level),
        special_level(other.special_level),
        special_points(other.special_points),
        max_refine_level(other.max_refine_level),
        truncate_mode(other.truncate_mode), autorefine(other.autorefine),
        truncate_on_project(other.truncate_on_project), targs(other.targs),
        cdata(FunctionCommonData<T, NDIM>::get(k)), functor(),
  ...
  void replicate(bool fence = true) { coeffs.replicate(fence); }

  void
  distribute(std::shared_ptr<WorldDCPmapInterface<Key<NDIM>>> newmap) const {
  ...
  void copy_coeffs(const FunctionImpl<Q, NDIM> &other, bool fence) {
    typename FunctionImpl<Q, NDIM>::dcT::const_iterator end =
        other.coeffs.end();
    for (typename FunctionImpl<Q, NDIM>::dcT::const_iterator it =
             other.coeffs.begin();
         it != end; ++it) {
  ...
  template <typename Q, typename R>
  void gaxpy_inplace_reconstructed(const T &alpha,
                                   const FunctionImpl<Q, NDIM> &g,
                                   const R &beta, const bool fence) {
    // merge g's tree into this' tree
    this->merge_trees(beta, g, alpha, true);

    // sum down the sum coeffs into the leafs
    if (world.rank() == coeffs.owner(cdata.key0))
      sum_down_spawn(cdata.key0, coeffT());
    if (fence)
      world.gop.fence();
  }

  /// merge the trees of this and other, while multiplying them with the alpha
  /// or beta, resp

  /// first step in an inplace gaxpy operation for reconstructed functions;
  /// assuming the same distribution for this and other
  ...
  template <typename Q, typename R>
  void merge_trees(const T alpha, const FunctionImpl<Q, NDIM> &other,
                   const R beta, const bool fence = true) {
    MADNESS_ASSERT(get_pmap() == other.get_pmap());
    other.flo_unary_op_node_inplace(do_merge_trees<Q, R>(alpha, beta, *this),
                                    fence);
    if (fence)
      world.gop.fence();
  }
  ...
  /// result and rhs do not have to have the same distribution or live in the
  /// same world result+=alpha* this
  /// @param[in]	alpha	prefactor for this
  template <typename Q, typename R>
  void accumulate_trees(FunctionImpl<Q, NDIM> &result, const R alpha,
  ...
  /// nothing returned, but leaves this's tree reconstructed and as sum of f and
  /// g
  ...
  template <typename Q, typename R> struct do_gaxpy_inplace {
    ...
    bool operator()(typename rangeT::iterator &it) const {
      ...
      f->coeffs.send(key, &nodeT::template gaxpy_inplace<Q, R>, alpha,
                     other_node, beta);
      return true;
    }
    template <typename Archive> void serialize(Archive &ar) {
  ...
  /// @param[in]  beta    prefactor for other
  template <typename Q, typename R>
  void gaxpy_inplace(const T &alpha, const FunctionImpl<Q, NDIM> &other,
                     const R &beta, bool fence) {
    //            MADNESS_ASSERT(get_pmap() == other.get_pmap());
    if (alpha != T(1.0))
      scale_inplace(alpha, false);
    typedef Range<typename FunctionImpl<Q, NDIM>::dcT::const_iterator> rangeT;
    typedef do_gaxpy_inplace<Q, R> opT;
    other.world.taskq.template for_each<rangeT, opT>(
        rangeT(other.coeffs.begin(), other.coeffs.end()),
        opT(this, T(1.0), beta));
    if (fence)
  ...
  template <typename Archive> void load(Archive &ar) {
    ...
    ar & thresh & initial_level & max_refine_level & truncate_mode &
        autorefine & truncate_on_project &
        tree_state; // nonstandard & compressed ; //& bc;
  ...
  template <typename Archive> void store(Archive &ar) {
    // WE RELY ON K BEING STORED FIRST
  ...
  void set_tree_state(const TreeState &state) { tree_state = state; }

  TreeState get_tree_state() const { return tree_state; }

  void set_functor(
      const std::shared_ptr<FunctionFunctorInterface<T, NDIM>> functor1);
  ...
  void accumulate_timer(
      const double time) const; // !!!!!!!!!!!!  REDUNDANT !!!!!!!!!!!!!!!
  ...
  /// @param[in] key the key to the current function node being evaluated for
  /// truncation
  /// @param[in] tol the tolerance for thresholding
  /// @param[in] v vector of Future<bool>'s that specify whether the current
  /// nodes children have coeffs
  bool truncate_op(const keyT &key, double tol,
  ...
  void fcube(const keyT &key, const FunctionFunctorInterface<T, NDIM> &f,
             const Tensor<double> &qx, tensorT &fval) const;
  ...
  /// @param[in] qx quadrature points on a level=0 box
  /// @param[out] fval values
  void fcube(const keyT &key, T (*f)(const coordT &), const Tensor<double> &qx,
             tensorT &fval) const;
  ...
  /// @param[in] maxlevel the maximum level of the tree for printing
  /// @param[out] os the ostream to where the output is sent
  void print_tree_graphviz(std::ostream &os = std::cout,
                           Level maxlevel = 10000) const;

  /// Functor for the do_print_tree method (using GraphViz)
  void do_print_tree_graphviz(const keyT &key, std::ostream &os,
                              Level maxlevel) const;

  /// Same as print_tree() but in JSON format
  /// @param[out] os the ostream to where the output is sent
  /// @param[in] maxlevel the maximum level of the tree for printing
  void print_tree_json(std::ostream &os = std::cout,
                       Level maxlevel = 10000) const;

  /// Functor for the do_print_tree_json method
  void
  do_print_tree_json(const keyT &key,
  ...
  void print_plane(const std::string filename, const int xaxis, const int yaxis,
                   const coordT &el2);

  /// collect the data for a plot of the MRA structure locally on each node

  /// @param[in]	xaxis	the x-axis in the plot (can be any axis of the
  /// MRA box)
  /// @param[in]	yaxis	the y-axis in the plot (can be any axis of the
  /// MRA box)
  /// @param[in]	el2     needs a description
  /// \todo Provide a description for el2
  Tensor<double> print_plane_local(const int xaxis, const int yaxis,
                                   const coordT &el2);
  ...
  /// @param[in]	yaxis	the y-axis in the plot (can be any axis of the
  /// MRA box)
  void do_print_plane(const std::string filename,
                      std::vector<Tensor<double>> plotinfo, const int xaxis,
                      const int yaxis, const coordT el2);
  ...
  /// @param[in]	filename	where the quadrature points will be
  /// written to
  /// @param[in]	keys		all leaf keys
  void do_print_grid(const std::string filename,
                     const std::vector<keyT> &keys) const;

  /// read data from a grid

  /// @param[in]	keyfile		file with keys and grid points for each
  /// key
  /// @param[in]	gridfile 	file with grid points, w/o key, but with
  /// same ordering
  /// @param[in]	vnuc_functor	subtract the values of this functor if
  /// regularization is needed
  template <size_t FDIM>
  typename std::enable_if<NDIM == FDIM>::type read_grid(
      const std::string keyfile, const std::string gridfile,
    ...
    long ndata, ndata1;
    if (not(std::getline(kfile, line)))
      MADNESS_EXCEPTION("failed reading 1st line of key data", 0);
    if (not(std::istringstream(line) >> ndata))
      MADNESS_EXCEPTION("failed reading k", 0);
    if (not(std::getline(gfile, line)))
      MADNESS_EXCEPTION("failed reading 1st line of grid data", 0);
    if (not(std::istringstream(line) >> ndata1))
      MADNESS_EXCEPTION("failed reading k", 0);
    MADNESS_CHECK(ndata == ndata1);
    if (not(std::getline(kfile, line)))
      MADNESS_EXCEPTION("failed reading 2nd line of key data", 0);
    if (not(std::getline(gfile, line)))
      MADNESS_EXCEPTION("failed reading 2nd line of grid data", 0);
    ...
    // the number of coordinates (grid point tuples) per box
    // ({x1},{x2},{x3},..,{xNDIM})
    ...
    while (std::getline(kfile, kline)) {
        ...
        for (size_t i = 0; i < npt; ++i) {
          ...
          for (size_t j = 0; j < npt; ++j) {
            ...
            for (size_t k = 0; k < npt; ++k) {
              ...
              //								fprintf(pFile,"%18.12f
              //%18.12f %18.12f\n",c[0],c[1],c[2]);
              auto &success1 = std::getline(gfile, gline);
              MADNESS_CHECK(success1);
              auto &success2 = std::getline(kfile, kline);
      ...
      const_cast<dcT &>(coeffs).send(
          key.parent(), &FunctionNode<T, NDIM>::set_has_children_recursive,
          coeffs, key.parent());
      ii++;
    }
  ...
  /// @param[in]	gridfile		file with keys and grid points
  /// and values for each key
  /// @param[in]	vnuc_functor	subtract the values of this functor if
  /// regularization is needed
  template <size_t FDIM>
  typename std::enable_if<NDIM == FDIM>::type read_grid2(
  ...
  /// @param[in] specialpts vector of special points in the function where we
  /// need
  ...
  void phi_for_mul(Level np, Translation lp, Level nc, Translation lc,
                   Tensor<double> &phi) const;
  ...
  /// @return 	coeffs
  const coeffT parent_to_child(const coeffT &s, const keyT &parent,
                               const keyT &child) const;
  ...
  /// @param[in] coeff the tensor of scaling function coefficients for function
  /// node (box)
  /// @return function values for function node (box)
  template <typename Q>
  GenTensor<Q> coeffs2values(const keyT &key, const GenTensor<Q> &coeff) const {
    // PROFILE_MEMBER_FUNC(FunctionImpl); // Too fine grain for routine
    // profiling
    double scale = pow(2.0, 0.5 * NDIM * key.level()) /
  ...
  /// equivalent to unfiltering the NS coeffs and then converting all child
  /// S-coeffs to values in their respective boxes. If only S coeffs are
  /// provided d coeffs are assumed to be zero. Reverse operation to
  /// values2NScoeffs().
  /// @param[in]	key	the key of the current S or NS coeffs, level n
  /// @param[in]	coeff coeffs in S or NS form; if S then d coeffs are
  /// assumed zero
  /// @param[in]	s_only	sanity check to avoid unintended discard of d
  /// coeffs
  /// @return		function values on the quadrature points of the children
  /// of child (!)
  template <typename Q>
  GenTensor<Q> NScoeffs2values(const keyT &key, const GenTensor<Q> &coeff,
    ...
    MADNESS_ASSERT((coeff.dim(0) == this->get_k()) or
                   (coeff.dim(0) == 2 * this->get_k()));

    // this is a block-diagonal matrix with the quadrature points on the
    // diagonal
    ...
    // the transformation matrix unfilters (cdata.hg) and transforms to values
    // in one step
    const Tensor<double> transf =
        (s_only) ? inner(cdata.hg(Slice(0, k - 1), _), quad_phit_2k) // S coeffs
  ...
  GenTensor<Q> NS_fcube_for_mul(const keyT &child, const keyT &parent,
    ...
    //            NScoeffs2values(child,coeff,s_only);
    ...
    if (s_only) {
      ...
      for (std::size_t d = 0; d < NDIM; ++d) {
        ...
        // input is S coeffs (dimension k), output is values on 2*npt grid
        // points
        ...
        phi_for_mul(parent.level(), parent.translation()[d], child.level() + 1,
                    2 * child.translation()[d], phi1);
        quad_phi[d](_, Slice(0, k - 1)) = phi1;

        // next the right child on level n+1 and translations 2l+1
        phi_for_mul(parent.level(), parent.translation()[d], child.level() + 1,
                    2 * child.translation()[d] + 1, phi1);
        quad_phi[d](_, Slice(k, 2 * k - 1)) = phi1;
      }

      const double scale =
          1.0 / sqrt(FunctionDefaults<NDIM>::get_cell_volume());
  ...
  /// @param[in]	values	tensor holding function values of the 2^NDIM
  /// children of key
  /// @return		NS coeffs belonging to key
  template <typename Q>
  GenTensor<Q> values2NScoeffs(const keyT &key,
                               const GenTensor<Q> &values) const {
    // PROFILE_MEMBER_FUNC(FunctionImpl);  // Too fine grain for routine
    // profiling
  ...
  /// Return the scaling function coeffs when given the function values at the
  /// quadrature points
  /// @param[in] key the key of the function node (box)
  /// @return function values for function node (box)
  template <typename Q>
  ...
  template <typename Q>
  GenTensor<Q> values2coeffs(const keyT &key,
                             const GenTensor<Q> &values) const {
    // PROFILE_MEMBER_FUNC(FunctionImpl); // Too fine grain for routine
    // profiling
  ...
  /// @param[in] coeff the coefficients of scaling function basis of the parent
  /// box
  template <typename Q>
  Tensor<Q> fcube_for_mul(const keyT &child, const keyT &parent,
                          const Tensor<Q> &coeff) const {
    // PROFILE_MEMBER_FUNC(FunctionImpl); // Too fine grain for routine
    // profiling
    if (child.level() == parent.level()) {
      ...
      for (std::size_t d = 0; d < NDIM; ++d) {
        ...
        phi_for_mul(parent.level(), parent.translation()[d], child.level(),
                    child.translation()[d], phi[d]);
      }
      return general_transform(coeff, phi)
          .scale(1.0 / sqrt(FunctionDefaults<NDIM>::get_cell_volume()));
      ;
    }
  }
  ...
  /// box
  template <typename Q>
  GenTensor<Q> fcube_for_mul(const keyT &child, const keyT &parent,
                             const GenTensor<Q> &coeff) const {
    // PROFILE_MEMBER_FUNC(FunctionImpl); // Too fine grain for routine
    // profiling
    if (child.level() == parent.level()) {
      return coeffs2values(parent, coeff);
  ...
  void do_mul(const keyT &key, const Tensor<L> &left,
    ...
    TERNARY_OPTIMIZED_ITERATOR(T, tcube, L, lcube, R, rcube,
                               *_p0 = *_p1 * *_p2;);
    double scale = pow(0.5, 0.5 * NDIM * key.level()) *
                   sqrt(FunctionDefaults<NDIM>::get_cell_volume());
  ...
  /// multiply the values of two coefficient tensors using a custom number of
  /// grid points
  ...
  /// @param[in]	npt	number of grid points (optional, default is
  /// cdata.npt)
  /// @return		coefficient tensor holding the product of the values of
  /// c1 and c2
  template <typename R>
  Tensor<TENSOR_RESULT_TYPE(T, R)> mul(const Tensor<T> &c1, const Tensor<R> &c2,
    ...
    const FunctionCommonData<T, NDIM> &cdata2 =
        FunctionCommonData<T, NDIM>::get(npt);
    ...
    TERNARY_OPTIMIZED_ITERATOR(resultT, resultvalue, T, c1value, R, c2value,
                               *_p0 = *_p1 * *_p2;);
  ...
  void do_binary_op(const keyT &key, const Tensor<L> &left,
                    const std::pair<keyT, Tensor<R>> &arg, const opT &op) {
    // PROFILE_MEMBER_FUNC(FunctionImpl); // Too fine grain for routine
    // profiling
    const keyT &rkey = arg.first;
    const Tensor<R> &rcoeff = arg.second;
  ...
  /// Invoked by result to perform result += alpha*left+beta*right in wavelet
  /// basis

  /// Does not assume that any of result, left, right have the same
  /// distribution. For most purposes result will start as an empty so actually
  /// are implementing out of place gaxpy.  If all functions have the same
  /// distribution there is no communication except for the optional fence.
  template <typename L, typename R>
  void gaxpy(T alpha, const FunctionImpl<L, NDIM> &left, T beta,
             const FunctionImpl<R, NDIM> &right, bool fence) {
    ...
    // not miss any nodes.
    typename FunctionImpl<L, NDIM>::dcT::const_iterator left_end =
        left.coeffs.end();
    for (typename FunctionImpl<L, NDIM>::dcT::const_iterator it =
             left.coeffs.begin();
         it != left_end; ++it) {
      const keyT &key = it->first;
      const typename FunctionImpl<L, NDIM>::nodeT &other_node = it->second;
      coeffs.send(key, &nodeT::template gaxpy_inplace<T, L>, 1.0, other_node,
                  alpha);
    }
    typename FunctionImpl<R, NDIM>::dcT::const_iterator right_end =
        right.coeffs.end();
    for (typename FunctionImpl<R, NDIM>::dcT::const_iterator it =
             right.coeffs.begin();
         it != right_end; ++it) {
      const keyT &key = it->first;
      const typename FunctionImpl<L, NDIM>::nodeT &other_node = it->second;
      coeffs.send(key, &nodeT::template gaxpy_inplace<T, R>, 1.0, other_node,
                  beta);
    }
    if (fence)
      world.gop.fence();
  ...
  /// Unary operation applied inplace to the coefficients WITHOUT refinement,
  /// optional fence
  ...
  /// Integrate over one particle of a two particle function and get a one
  /// particle function bsp \int g(1,2) \delta(2-1) d2 = f(1) The overall
  /// dimension of g should be even

  /// The operator
  template <std::size_t LDIM>
  void dirac_convolution_op(const keyT &key, const nodeT &node,
                            FunctionImpl<T, LDIM> *f) const {
    ...
    // break the 6D key into two 3D keys (may also work for every even
    // dimension)
    Key<LDIM> key1, key2;
    ...
    tensorT vec1 = copy(
        g_values.get_svdtensor().ref_vector(0).reshape(rank, maxk, maxk, maxk));
    tensorT vec2 =
        g_values.get_svdtensor().ref_vector(1).reshape(rank, maxk, maxk, maxk);
    ...
    for (long i = 0; i < rank; ++i) {
      ...
      c1.emul(c2); // this changes vec1 because of shallow copy, but not the g
                   // function because of the deep copy made above
      double singular_value_i = g_values.get_svdtensor().weights(i);
      result += (singular_value_i * c1);
    }

    // accumulate coefficients (since only diagonal boxes are used the
    // coefficients get just replaced, but accumulate is needed to create the
    // right tree structure
    tensorT f_coeff = f->values2coeffs(key1, result);
    f->coeffs.task(key1, &FunctionNode<T, LDIM>::accumulate2, f_coeff,
                   f->coeffs, key1, TaskAttributes::hipri());
  ...
  void do_dirac_convolution(FunctionImpl<T, LDIM> *f, bool fence) const {
    ...
    for (typename dcT::const_iterator it = this->coeffs.begin(); it != end;
         ++it) {
      // looping through all the leaf(!) coefficients in the NDIM function
      // ("this")
      const keyT &key = it->first;
      ...
      if (node.is_leaf()) {
        ...
        if (key1 == key2) {
          ...
          woT::task(p, &implT::template dirac_convolution_op<LDIM>, key, node,
                    f);
        }
      }
    }
    world.gop.fence(); // fence is necessary if trickle down is used afterwards
    // trickle down and undo redundand shouldnt change anything if only the
    // diagonal elements are considered above -> check this
    f->trickle_down(
        true); // fence must be true otherwise undo_redundant will have trouble
               //            f->undo_redundant(true);
    f->verify_tree();
    // if (fence) world.gop.fence(); // unnecessary, fence is activated in
    // undo_redundant
  }
  ...
  /// given an NS tree resulting from a convolution, truncate leafs if
  /// appropriate
  struct do_truncate_NS_leafs {
    ...
    bool operator()(typename rangeT::iterator &it) const {
      ...
      if (node.is_leaf() and node.coeff().has_data()) {
        ...
        if (error < tol)
          node.coeff() = copy(node.coeff()(f->cdata.s0));
      }
      return true;
    }
    template <typename Archive> void serialize(const Archive &ar) {}
  ...
  struct do_reduce_rank {
    ...
    do_reduce_rank(const double &thresh) { args.thresh = thresh; }
    ...
    double operator()(typename rangeT::iterator &it) const {
      ...
      if constexpr (NDIM == 6) {
        ...
        if (f->get_coeffs().probe(mapkey)) {
          ...
          //                    bool have_c1=fnode.coeff().has_data() and
          //                    fnode.coeff().config().has_data(); bool
          //                    have_c2=mapnode.coeff().has_data() and
          //                    mapnode.coeff().config().has_data();
  ...
  /// result+= alpha*this
  /// this and result don't have to have the same distribution or live in the
  /// same world no comm, and the tree should be in an consistent state by
  /// virtue
  template <typename Q, typename R> struct do_accumulate_trees {
    typedef Range<typename dcT::const_iterator> rangeT;
    ...
    bool operator()(typename rangeT::iterator &it) const {
      ...
      if (node.has_coeff())
        result->get_coeffs().task(key, &nodeT::accumulate, alpha * node.coeff(),
  ...
  template <typename Q, typename R> struct do_merge_trees {
    typedef Range<typename dcT::const_iterator> rangeT;
  ...
  struct do_mapdim {
    ...
    bool operator()(typename rangeT::iterator &it) const {
      ...
      f->get_coeffs().replace(keyT(key.level(), l),
                              nodeT(cc, node.has_children()));
  ...
  struct do_map_and_mirror {
    ...
    do_map_and_mirror(const std::vector<long> map,
                      const std::vector<long> mirror, implT &f)
  ...
  template <typename opT> struct do_unary_op_value_inplace {
    typedef Range<typename dcT::iterator> rangeT;
    implT *impl;
    opT op;
  ...
  void vtransform_doit(
      const std::shared_ptr<FunctionImpl<R, NDIM>> &right, const Tensor<Q> &c,
    ...
    typename FunctionImpl<R, NDIM>::dcT::const_iterator end =
        right->coeffs.end();
    for (typename FunctionImpl<R, NDIM>::dcT::const_iterator it =
             right->coeffs.begin();
         it != end; ++it) {
          ...
          if (std::abs(norm * c(i)) > keytol) {
            ...
            if (newnode && key.level() > 0) {
              ...
              if (left->coeffs.is_local(parent))
                left->coeffs.send(parent, &nodeT::set_has_children_recursive,
                                  left->coeffs, parent);
              else
                left->coeffs.task(parent, &nodeT::set_has_children_recursive,
                                  left->coeffs, parent);
            }
  ...
  template <typename Q, typename R>
  /// @todo I don't know what this does other than a trasform
  void vtransform_fast_doit(
      const keyT &node_key,
      const std::vector<std::shared_ptr<FunctionImpl<R, NDIM>>> &v_input,
      const Tensor<Q> &c, const double tol,
      const std::vector<std::shared_ptr<FunctionImpl<T, NDIM>>> &vresult) {
    // print("vtransform_fast_doit", key);

    // Loop thru input functions
    for (size_t mu = 0; mu < v_input.size(); ++mu) {
      typename dcT::const_accessor accright;
      // If the function has a node at this key
      if (v_input[mu]->get_coeffs().find(accright, node_key)) {
        const auto &input_node = accright->second;
        const auto &input_coeffs = input_node.coeff(); // might be empty
        // Loop thru output functions
        for (size_t nu = 0; nu < vresult.size(); ++nu) {
          typename dcT::accessor accresult;
          // Try to insert an empty node at this key
          if (vresult[nu]->get_coeffs().insert(accresult, node_key)) {
            // print("   inserted", key);
          }
          auto &resultnode = accresult->second;
          // Ensure that the left node has children if the right node does, no
          // need to worry about parents
          if (input_node.has_children())
            resultnode.set_has_children(true);
          // If the right node has coefficients then do the operation
          if (input_node.has_coeff()) {
            // If the left node has no coefficients then create them
            if (!resultnode.has_coeff()) {
              resultnode.set_coeff(coeffT(cdata.v2k, targs));
              // print("   set coeff", key, accleft->second.has_coeff());
            }
            auto &result_coeffs = accresult->second.coeff();
            result_coeffs.gaxpy(1.0, input_coeffs, c(nu, mu));
  ...
  void refine_to_common_level(const std::vector<FunctionImpl<T, NDIM> *> &v,
                              const std::vector<tensorT> &c, const keyT key);

  /// Inplace operate on many functions (impl's) with an operator within a
  /// certain box
  ...
  template <typename opT>
  void multiop_values_doit(const keyT &key, const opT &op,
                           const std::vector<implT *> &v) {
    std::vector<tensorT> c(v.size());
    for (unsigned int i = 0; i < v.size(); i++) {
      if (v[i]) {
        coeffT cc =
            coeffs2values(key, v[i]->coeffs.find(key).get()->second.coeff());
        c[i] = cc.full_tensor();
      }
    }
    tensorT r = op(key, c);
  ...
  /// Inplace operate on many functions (impl's) with an operator within a
  /// certain box Assumes all functions have been refined down to the same level
  /// @param[in] op the operator
  /// @param[in] v the vector of function impl's on which to be operated
  template <typename opT>
  void multiop_values(const opT &op, const std::vector<implT *> &v) {
    ...
    for (typename dcT::iterator it = v[0]->coeffs.begin(); it != end; ++it) {
      const keyT &key = it->first;
      if (it->second.has_coeff())
        world.taskq.add(*this, &implT::template multiop_values_doit<opT>, key,
                        op, v);
      else
  ...
  void multi_to_multi_op_values_doit(const keyT &key, const opT &op,
    ...
    for (unsigned int i = 0; i < vin.size(); i++) {
      if (vin[i]) {
        coeffT cc =
            coeffs2values(key, vin[i]->coeffs.find(key).get()->second.coeff());
        c[i] = cc.full_tensor();
      }
    }
    ...
    for (std::size_t i = 0; i < vout.size(); ++i) {
      vout[i]->coeffs.replace(
          key, nodeT(coeffT(values2coeffs(key, r[i]), targs), false));
    }
  }
  ...
  void multi_to_multi_op_values(const opT &op, const std::vector<implT *> &vin,
    ...
    for (typename dcT::iterator it = vin[0]->coeffs.begin(); it != end; ++it) {
      const keyT &key = it->first;
      if (it->second.has_coeff())
        world.taskq.add(*this,
                        &implT::template multi_to_multi_op_values_doit<opT>,
                        key, op, vin, vout);
      else {
  ...
  /// Transforms a vector of functions left[i] = sum[j] right[j]*c[j,i] using
  /// sparsity
  ...
  template <typename Q, typename R>
  void
  vtransform(const std::vector<std::shared_ptr<FunctionImpl<R, NDIM>>> &vright,
             const Tensor<Q> &c,
             const std::vector<std::shared_ptr<FunctionImpl<T, NDIM>>> &vleft,
    ...
    for (const auto &key : keys) {
      world.taskq.add(*this, &implT::template vtransform_fast_doit<Q, R>, key,
                      vright, c, tol, vleft);
    }
    ...
    // if (old) {
    //     for (unsigned int i=0; i<vleft.size(); ++i) {
    //         world.taskq.add(*this, &implT:: template vtransform_doit<Q,R>,
    //         vright, c, vleft[i], tol);
    //     }
    //     if (fence)
    //         world.gop.fence();
    //     return;
  ...
  /// Unary operation applied inplace to the values with optional refinement and
  /// fence
  ...
  void unary_op_value_inplace(const opT &op, bool fence) {
    typedef Range<typename dcT::iterator> rangeT;
    typedef do_unary_op_value_inplace<opT> xopT;
    world.taskq.for_each<rangeT, xopT>(rangeT(coeffs.begin(), coeffs.end()),
                                       xopT(this, op));
    if (fence)
      world.gop.fence();
  }
  ...
  /// @param[in] vrcin the vector scaling function coefficients associated with
  /// the
  ///            current box in the right functions
  /// @param[out] vresultin the vector of resulting functions (impl's)
  template <typename L, typename R>
  void mulXXveca(const keyT &key, const FunctionImpl<L, NDIM> *left,
                 const Tensor<L> &lcin,
    ...
    for (unsigned int i = 0; i < vrightin.size(); ++i) {
      ...
      if (rc.size() && lc.size()) { // Yipee!
        result->task(world.rank(), &implT::template do_mul<L, R>, key, lc,
                     std::make_pair(key, rc));
      } else if (tol && lnorm * rnorm < truncate_tol(tol, key)) {
        result->coeffs.replace(
            key, nodeT(coeffT(cdata.vk, targs), false)); // Zero leaf
      } else {                                           // Interior node
    ...
    if (vresult.size()) {
      ...
      for (KeyChildIterator<NDIM> kit(key); kit; ++kit) {
        ...
        woT::task(coeffs.owner(child), &implT::template mulXXveca<L, R>, child,
                  left, ll, vright, vv, vresult, tol);
      }
    }
  }
  ...
  template <typename L, typename R>
  void mulXXa(const keyT &key, const FunctionImpl<L, NDIM> *left,
              const Tensor<L> &lcin, const FunctionImpl<R, NDIM> *right,
              const Tensor<R> &rcin, double tol) {
    typedef typename FunctionImpl<L, NDIM>::dcT::const_iterator literT;
    typedef typename FunctionImpl<R, NDIM>::dcT::const_iterator riterT;
    ...
    if (tol) {
      ...
      if (lnorm * rnorm < truncate_tol(tol, key)) {
        coeffs.replace(key,
                       nodeT(coeffT(cdata.vk, targs), false)); // Zero leaf node
        return;
      }
    }
    ...
    for (KeyChildIterator<NDIM> kit(key); kit; ++kit) {
      ...
      woT::task(coeffs.owner(child), &implT::template mulXXa<L, R>, child, left,
                ll, right, rr, tol);
    }
  }

  // Binary operation on values using recursive descent and assuming same
  // distribution
  /// Both left and right functions are in the scaling function basis
  /// @param[in] key the key to the current function node (box)
  /// @param[in] left the function impl associated with the left function
  ...
  template <typename L, typename R, typename opT>
  void binaryXXa(const keyT &key, const FunctionImpl<L, NDIM> *left,
                 const Tensor<L> &lcin, const FunctionImpl<R, NDIM> *right,
                 const Tensor<R> &rcin, const opT &op) {
    typedef typename FunctionImpl<L, NDIM>::dcT::const_iterator literT;
    typedef typename FunctionImpl<R, NDIM>::dcT::const_iterator riterT;
    ...
    for (KeyChildIterator<NDIM> kit(key); kit; ++kit) {
      ...
      woT::task(coeffs.owner(child), &implT::template binaryXXa<L, R, opT>,
                child, left, ll, right, rr, op);
    }
  }

  template <typename Q, typename opT> struct coeff_value_adaptor {
  ...
  void unaryXXa(const keyT &key, const FunctionImpl<Q, NDIM> *func,
                const opT &op) {

    //            const Tensor<Q>& fc =
    //            func->coeffs.find(key).get()->second.full_tensor_copy();
    const Tensor<Q> fc =
    ...
    if (fc.size() == 0) {
      ...
      for (KeyChildIterator<NDIM> kit(key); kit; ++kit) {
        const keyT &child = kit.key();
        woT::task(coeffs.owner(child), &implT::template unaryXXa<Q, opT>, child,
                  func, op);
      }
    } else {
      tensorT t = op(key, fc);
  ...
  /// Multiplies two functions (impl's) together. Delegates to the mulXXa()
  /// method
  ...
  template <typename L, typename R>
  void mulXX(const FunctionImpl<L, NDIM> *left,
             const FunctionImpl<R, NDIM> *right, double tol, bool fence) {
    if (world.rank() == coeffs.owner(cdata.key0))
  ...
  /// Performs binary operation on two functions (impl's). Delegates to the
  /// binaryXXa() method
  /// @param[in] left pointer to the left function impl
  /// @param[in] right pointer to the right function impl
  /// @param[in] op the binary operator
  template <typename L, typename R, typename opT>
  void binaryXX(const FunctionImpl<L, NDIM> *left,
                const FunctionImpl<R, NDIM> *right, const opT &op, bool fence) {
    if (world.rank() == coeffs.owner(cdata.key0))
  ...
  /// Performs unary operation on function impl. Delegates to the unaryXXa()
  /// method
  ...
  /// @param[in] op the unary operator
  template <typename Q, typename opT>
  void unaryXXvalues(const FunctionImpl<Q, NDIM> *func, const opT &op,
                     bool fence) {
    if (world.rank() == coeffs.owner(cdata.key0))
  ...
  /// Multiplies a function (impl) with a vector of functions (impl's).
  /// Delegates to the mulXXveca() method.
  /// @param[in] left pointer to the left function impl
  ...
  void mulXXvec(const FunctionImpl<L, NDIM> *left,
                ...
                const std::vector<FunctionImpl<T, NDIM> *> &vresult, double tol,
                bool fence) {
  ...
  /// Three possibilities.
  ///
  /// 1) The coeffs are present and returned with the key of the containing
  /// node.
  ///
  ...
  /// @todo Robert .... help!
  void sock_it_to_me_too(
      const keyT &key,
      const RemoteReference<FutureImpl<std::pair<keyT, coeffT>>> &ref) const;

  /// @todo help!
  void plot_cube_kernel(archive::archive_ptr<Tensor<T>> ptr, const keyT &key,
                        const coordT &plotlo, const coordT &plothi,
                        const std::vector<long> &npt, bool eval_refine) const;

  /// Evaluate a cube/slice of points ... plotlo and plothi are already in
  /// simulation coordinates No communications
  ...
  Tensor<T> eval_plot_cube(const coordT &plotlo, const coordT &plothi,
  ...
  std::pair<bool, T> eval_local_only(const Vector<double, NDIM> &xin,
                                     Level maxlevel);
  ...
  /// to other nodes.
  void eval(const Vector<double, NDIM> &xin, const keyT &keyin,
  ...
  void evaldepthpt(const Vector<double, NDIM> &xin, const keyT &keyin,
                   const typename Future<Level>::remote_refT &ref);

  /// Get the rank of leaf box of the tree at a point in \em simulation
  /// coordinates
  ...
  ///
  /// This function is a minimally-modified version of eval()
  void evalR(const Vector<double, NDIM> &xin, const keyT &keyin,
  ...
  void static tnorm(const SVDTensor<T> &t, double *lo, double *hi,
                    const int particle);
  ...
  void do_square_inplace2(const keyT &parent, const keyT &child,
  ...
  /// After 1d push operator must sum coeffs down the tree to restore correct
  /// scaling function coefficients
  void sum_down(bool fence);

  /// perform this multiplication: h(1,2) = f(1,2) * g(1)
  template <size_t LDIM> struct multiply_op {
    ...
    bool screen(const coeffT &fcoeff, const coeffT &gcoeff,
    ...
    /// @param[in]  key key for FunctionNode in f and g, (g: broken into
    /// particles)
    ...
    std::pair<bool, coeffT> operator()(const Key<NDIM> &key) const {
      ...
      //                if (not is_leaf) return std::pair<bool,coeffT>
      //                (is_leaf,coeffT());

      // break key into particles (these are the child keys, with f/gdatum come
      // the parent keys)
      Key<LDIM> key1, key2;
      key.break_apart(key1, key2);
      ...
      coeff1.normalize();
      const coeffT coeff2 =
          g.get_impl()->parent_to_child(g.coeff(), g.key(), gkey);
    ...
    Future<this_type> activate() const {
      ...
      return h->world.taskq.add(
          detail::wrap_mem_fn(*const_cast<this_type *>(this),
                              &this_type::forward_ctor),
          h, f1, g1, particle);
    }

    this_type forward_ctor(implT *h1, const ctT &f1, const ctL &g1,
                           const int particle) {
  ...
  struct add_op {
    ...
    Future<this_type> activate() const {
      ...
      return f.get_impl()->world.taskq.add(
          detail::wrap_mem_fn(*const_cast<this_type *>(this),
                              &this_type::forward_ctor),
          f1, g1, alpha, beta);
    }
    ...
    this_type forward_ctor(const ctT &f1, const ctT &g1, const double alpha,
                           const double beta) {
  ...
  void multiply(const implT *f, const FunctionImpl<T, LDIM> *g,
                const int particle) {
    ...
    if (world.rank() == coeffs.owner(key0)) {
      ...
      woT::task(p, &implT::template forward_traverse<coeff_opT, apply_opT>,
                coeff_op, apply_op, key0);
    }
  ...
  template <size_t LDIM, typename leaf_opT> struct hartree_op {
    bool randomize() const { return false; }
    ...
    // ctor
    hartree_op() : result() {}
    hartree_op(implT *result, const ctL &p11, const ctL &p22,
    ...
    std::pair<bool, coeffT> operator()(const Key<NDIM> &key) const {
      ...
      // break key into particles (these are the child keys, with datum1/2 come
      // the parent keys)
      Key<LDIM> key1, key2;
      key.break_apart(key1, key2);
    ...
    this_type make_child(const keyT &child) const {
      ...
      return this_type(result, p1.make_child(key1), p2.make_child(key2),
                       leaf_op);
    }
    ...
    this_type forward_ctor(implT *result1, const ctL &p11, const ctL &p22,
                           const leaf_opT &leaf_op) {
  ...
  /// part II: activate coeff_op, i.e. retrieve all the necessary remote boxes
  /// (communication)
  /// @param[in]	coeff_op	operator making the coefficients that
  /// needs activation
  /// @param[in]	apply_op	just passing thru
  /// @param[in]	key			the key we are working on
  template <typename coeff_opT, typename apply_opT>
  void forward_traverse(const coeff_opT &coeff_op, const apply_opT &apply_op,
                        const keyT &key) const {
    MADNESS_ASSERT(coeffs.is_local(key));
    Future<coeff_opT> active_coeff = coeff_op.activate();
    woT::task(world.rank(),
              &implT::template traverse_tree<coeff_opT, apply_opT>,
              active_coeff, apply_op, key);
  }

  /// traverse a non-existing tree

  /// part I: make the coefficients, process them and continue the recursion if
  /// necessary
  /// @param[in]	coeff_op	operator making the coefficients and
  /// determining them being leaves
  /// @param[in]	apply_op	operator processing the coefficients
  /// @param[in]	key			the key we are currently working
  /// on
  template <typename coeff_opT, typename apply_opT>
  void traverse_tree(const coeff_opT &coeff_op, const apply_opT &apply_op,
                     const keyT &key) const {
    MADNESS_ASSERT(coeffs.is_local(key));
    ...
    if (has_children) {
      for (KeyChildIterator<NDIM> kit(key); kit; ++kit) {
        ...
        void (implT::*ft)(const coeff_opT &, const apply_opT &, const keyT &)
  ...
  /// @param[in]	leaf_op	operator determining of a given box will be a
  /// leaf
  template <std::size_t LDIM, typename leaf_opT>
  void
  hartree_product(const std::vector<std::shared_ptr<FunctionImpl<T, LDIM>>> p1,
                  const std::vector<std::shared_ptr<FunctionImpl<T, LDIM>>> p2,
                  const leaf_opT &leaf_op, bool fence) {
    MADNESS_CHECK_THROW(p1.size() == p2.size(),
                        "hartree_product: p1 and p2 must have the same size");
    for (auto &p : p1)
      MADNESS_CHECK(p->is_nonstandard() or p->is_nonstandard_with_leaves());
    for (auto &p : p2)
      MADNESS_CHECK(p->is_nonstandard() or p->is_nonstandard_with_leaves());
  ...
  void apply_1d_realspace_push_op(const archive::archive_ptr<const opT> &pop,
                                  int axis, const keyT &key,
                                  const Tensor<R> &c) {
    const opT *op = pop.ptr;
    const Level n = key.level();
    const double cnorm = c.normf();
    const double tol =
        truncate_tol(thresh, key) * 0.1; // ??? why this value????
    ...
    for (Translation s = 0; s < maxs; ++s) {
      ...
      for (int direction = -1; direction <= maxdir; direction += 2) {
        ...
        if (lnew[axis] >= 0 &&
            lnew[axis] < maxs) { // NON-ZERO BOUNDARY CONDITIONS IGNORED HERE
                                 // !!!!!!!!!!!!!!!!!!!!
          const Tensor<typename opT::opT> &r =
              op->rnlij(n, s * direction, true);
          ...
          if (s <= 1 ||
              r.normf() * cnorm > tol) { // Always do kernel and neighbor
            nsmall = 0;
  ...
  template <typename opT, typename R>
  void apply_1d_realspace_push(const opT &op, const FunctionImpl<R, NDIM> *f,
                               int axis, bool fence) {
  ...
  void forward_do_diff1(const DerivativeBase<T, NDIM> *D, const implT *f,
                        const keyT &key, const std::pair<keyT, coeffT> &left,
                        const std::pair<keyT, coeffT> &center,
                        const std::pair<keyT, coeffT> &right);

  void do_diff1(const DerivativeBase<T, NDIM> *D, const implT *f,
                const keyT &key, const std::pair<keyT, coeffT> &left,
                const std::pair<keyT, coeffT> &center,
                const std::pair<keyT, coeffT> &right);
  ...
  keyT neighbor(const keyT &key, const keyT &disp,
                const std::vector<bool> &is_periodic) const;
  ...
  coeffT multiply(const coeffT &val_ket, const coeffT &val_pot,
                  int particle) const;
  ...
  /// or 						(v(1,2) + v(1) + v(2))
  /// |p(1) p(2)> i.e. coefficients for the ket and coefficients for the two
  /// particles are mutually exclusive. All potential terms are optional, just
  /// pass in empty coeffs.
  /// @param[in]	key			the key of the FunctionNode to
  /// which these coeffs belong
  /// @param[in]	coeff_ket	coefficients of the ket
  /// @param[in]	vpotential1	function values of the potential for
  /// particle 1
  /// @param[in]	vpotential2	function values of the potential for
  /// particle 2
  /// @param[in]	veri		function values for the 2-particle
  /// potential
  ...
  template <std::size_t LDIM> struct pointwise_multiplier {
    ...
    /// multiply values of rhs and lhs, result on rhs, rhs and lhs are of the
    /// same dimensions
    tensorT operator()(const Key<NDIM> key, const tensorT &coeff_rhs) {
      ...
      // the tnorm estimate is not tight enough to be efficient, better use
      // oversampling
      ...
      if (use_tnorm) {
        ...
        auto &cdata =
            FunctionCommonData<T, NDIM>::get(coeff_rhs.dim(0)); // npt=k+1
        auto &cdata_npt = FunctionCommonData<T, NDIM>::get(
            coeff_rhs.dim(0) + oversampling); // npt=k+1
    ...
    /// multiply values of rhs and lhs, result on rhs, rhs and lhs are of
    /// differnet dimensions
    coeffT operator()(const Key<NDIM> key, const tensorT &coeff_rhs,
                      const int particle) {
      Key<LDIM> key1, key2;
      key.break_apart(key1, key2);
      ...
      coeffT val_rhs;
      if (particle == 1)
        val_rhs = outer(val_rhs_npt1, ones, targs);
      if (particle == 2)
        val_rhs = outer(ones, val_rhs_npt1, targs);
  ...
  /// given a ket and the 1- and 2-electron potentials, construct the function V
  /// phi

  /// small memory footstep version of Vphi_op: use the NS form to have
  /// information about parent and children to determine if a box is a leaf.
  /// This will require compression of the constituent functions, which will
  /// lead to more memory usage there, but will avoid oversampling of the result
  /// function.
  template <typename opT, size_t LDIM> struct Vphi_op_NS {
    ...
    void accumulate_into_result(const Key<NDIM> &key,
                                const coeffT &coeff) const {
    ...
    Vphi_op_NS(implT *result, const opT &leaf_op, const ctT &iaket,
               const ctL &iap1, const ctL &iap2, const ctL &iav1,
               const ctL &iav2, const implT *eri)
        : result(result), leaf_op(leaf_op), iaket(iaket), iap1(iap1),
          iap2(iap2), iav1(iav1), iav2(iav2), eri(eri) {

      // 2-particle potential must be on-demand
      if (eri)
        MADNESS_ASSERT(eri->is_on_demand());
    }
    ...
    std::pair<bool, coeffT> operator()(const Key<NDIM> &key) const {
      ...
      if (leaf_op.do_pre_screening()) {
        // this means that we only construct the boxes which are leaf boxes from
        // the other function in the leaf_op
        if (leaf_op.pre_screening(key)) {
          ...
          return continue_recursion(std::vector<bool>(1 << NDIM, false),
                                    tensorT(), key);
        }
      }

      // this means that the function has to be completely constructed and not
      // mirrored by another function

      // if the initial level is not reached then this must not be a leaf box
      size_t il = result->get_initial_level();
      if (FunctionDefaults<NDIM>::get_refine())
        il += 1;
      if (key.level() < int(il)) {
        return continue_recursion(std::vector<bool>(1 << NDIM, false),
                                  tensorT(), key);
      }
      // if further refinement is needed (because we are at a special box,
      // special point) and the special_level is not reached then this must not
      // be a leaf box
      if (key.level() < result->get_special_level() and
      ...
      // coeffs are leaf (for whatever reason), insert into tree and stop
      // recursion
    ...
    /// loop over all children and either insert their sum coeffs or continue
    /// the recursion

    /// @param[in]	child_is_leaf	for each child: is it a leaf?
    /// @param[in]	coeffs	coefficient tensor with 2^N sum coeffs
    /// (=unfiltered NS coeffs)
    /// @param[in]	key		the key for the NS coeffs (=parent key
    /// of the children)
    /// @return		to avoid recursion outside this return:
    /// std::pair<is_leaf,coeff> = true,coeffT()
    std::pair<bool, coeffT>
    continue_recursion(const std::vector<bool> child_is_leaf,
      ...
      for (KeyChildIterator<NDIM> kit(key); kit; ++kit, ++i) {
        ...
        if (is_leaf) {
          ...
          iop(child,
              coeffT(copy(coeffs(result->child_patch(child))),
          ...
          void (implT::*ft)(const Vphi_op_NS<opT, LDIM> &,
                            const noop<T, NDIM> &, const keyT &) const =
              &implT::template forward_traverse<Vphi_op_NS<opT, LDIM>,
                                                noop<T, NDIM>>;
          result->task(p, ft, child_op, no, child);
        }
      }
      // return e sum coeffs; also return always is_leaf=true:
      // the recursion is continued within this struct, not outside in
      // traverse_tree!
      return std::pair<bool, coeffT>(true, coeffT());
    }
    ...
    /// the error is computed from the d coefficients of the constituent
    /// functions

    /// the result is h_n = P_n(f g), computed as h_n \approx Pn(f_n g_n)
    /// its error is therefore
    /// h_n = (f g)_n = ((Pn(f) + Qn(f)) (Pn(g) + Qn(g))
    ///               = Pn(fn gn) + Qn(fn gn) + Pn(f) Qn(g) + Qn(f) Pn(g) +
    ///               Qn(f) Pn(g)
    /// the first term is what we compute, the second term is estimated by tnorm
    /// (in another function), the third to last terms are estimated in this
    /// function by e.g.: Qn(f)Pn(g) < ||Qn(f)|| ||Pn(g)||
    ...
    std::pair<coeffT, double> make_sum_coeffs(const keyT &key) const {
      ...
      // bool
      // printme=(int(key.translation()[0])==int(std::pow(key.level(),2)/2)) and
      // 		(int(key.translation()[1])==int(std::pow(key.level(),2)/2))
      // and
      ...
      const coeffT coeff_ket = (iaket.get_impl())
                                   ? iaket.coeff(key)
    ...
    Future<this_type> activate() const {
      ...
      return result->world.taskq.add(
          detail::wrap_mem_fn(*const_cast<this_type *>(this),
                              &this_type::forward_ctor),
          result, leaf_op, iaket1, iap11, iap21, iav11, iav21, eri);
    }

    this_type forward_ctor(implT *result1, const opT &leaf_op,
                           const ctT &iaket1, const ctL &iap11,
                           const ctL &iap21, const ctL &iav11, const ctL &iav21,
                           const implT *eri1) {
      return this_type(result1, leaf_op, iaket1, iap11, iap21, iav11, iav21,
                       eri1);
    }
  ...
  /// this function must have been constructed using the
  /// CompositeFunctorInterface. The interface provides one- and two-electron
  /// potentials, and the ket, which are assembled to give V*phi.
  ...
  void make_Vphi(const opT &leaf_op, const bool fence = true) {
    ...
    MADNESS_CHECK_THROW(NDIM == LDIM * 2,
                        "make_Vphi only works for even dimensions");
    ...
    std::shared_ptr<FunctionFunctorInterface<T, NDIM>> func2(
        this->get_functor());
    this->unset_functor();
    ...
    if (fence)
      func->make_redundant(true); // no-op if already redundant
    MADNESS_CHECK_THROW(func->check_redundant(),
                        "make_Vphi requires redundant functions");
    ...
    // - FunctionNode::accumulate() uses buffer -> add the buffer contents to
    // the actual coefficients
    // - the operation constructs sum coefficients on all scales -> sum down to
    // get a well-defined tree-state
    if (fence) {
      world.gop.fence();
  ...
  void make_Vphi_only(const opT &leaf_op, FunctionImpl<T, NDIM> *ket,
                      ...
                      FunctionImpl<T, NDIM> *eri, const bool fence = true) {
  ...
  tensorT downsample(const keyT &key,
                     const std::vector<Future<coeffT>> &v) const;
  ...
  template <typename opT> void refine_op(const opT &op, const keyT &key) {
    ...
    MADNESS_CHECK(found);
    nodeT &node = acc->second;
    if (node.has_coeff() && key.level() < max_refine_level &&
        op(this, key, node)) {
  ...
  template <typename opT> void refine_spawn(const opT &op, const keyT &key) {
    nodeT &node = coeffs.find(key).get()->second;
    if (node.has_children()) {
      for (KeyChildIterator<NDIM> kit(key); kit; ++kit)
        woT::task(coeffs.owner(kit.key()), &implT::template refine_spawn<opT>,
                  op, kit.key(), TaskAttributes::hipri());
    } else {
  ...
  template <typename opT> void refine(const opT &op, bool fence) {
    if (world.rank() == coeffs.owner(cdata.key0))
      woT::task(coeffs.owner(cdata.key0), &implT::template refine_spawn<opT>,
                op, cdata.key0, TaskAttributes::hipri());
    if (fence)
      world.gop.fence();
  }
  ...
  /// sum all the contributions from all scales after applying an operator in
  /// mod-NS form
  ...
  /// @param[in] nonstandard	keep sum coeffs at all other levels, except
  /// leaves
  /// @param[in] keepleaves	keep sum coeffs (but no diff coeffs) at leaves
  /// @param[in] redundant    keep only sum coeffs at all levels, discard
  /// difference coeffs
  //        void compress(bool nonstandard, bool keepleaves, bool redundant,
  //        bool fence);
  void compress(const TreeState newstate, bool fence);

  /// Invoked on node where key is local
  Future<std::pair<coeffT, double>> compress_spawn(const keyT &key,
                                                   bool nonstandard,
                                                   bool keepleaves,
                                                   bool redundant1);
...
public:
  ...
  Future<coeffT> truncate_reconstructed_spawn(const keyT &key,
                                              const double tol);

  /// given the sum coefficients of all children, truncate or not

  /// @return     new sum coefficients (empty if internal, not empty, if new
  /// leaf); might delete its children
  coeffT truncate_reconstructed_op(const keyT &key,
                                   const std::vector<Future<coeffT>> &v,
                                   const double tol);

  /// calculate the wavelet coefficients using the sum coefficients of all child
  /// nodes

  /// also compute the norm tree for all nodes
  /// @param[in] key 	this's key
  /// @param[in] v 	sum coefficients of the child nodes
  /// @param[in] nonstandard  keep the sum coefficients with the wavelet
  /// coefficients
  /// @param[in] redundant    keep only the sum coefficients, discard the
  /// wavelet coefficients
  /// @return 		the sum coefficients
  std::pair<coeffT, double>
  compress_op(const keyT &key,
  ...
  /// @return         the sum coefficients
  std::pair<coeffT, double>
  make_redundant_op(const keyT &key,
                    const std::vector<Future<std::pair<coeffT, double>>> &v);
  ...
  /// laziness
  template <size_t OPDIM> struct do_op_args {
    Key<OPDIM> key, d;
    keyT dest;
    double tol, fac, cnorm;
    do_op_args() {}
    do_op_args(const Key<OPDIM> &key, const Key<OPDIM> &d, const keyT &dest,
               double tol, double fac, double cnorm)
  ...
  /// for fine-grain parallelism: call the apply method of an operator in a
  /// separate task

  /// @param[in]  op      the operator working on our function
  /// @param[in]  c       full rank tensor holding the NS coefficients
  /// @param[in]  args    laziness holding norm of the coefficients,
  /// displacement, destination, ..
  template <typename opT, typename R, size_t OPDIM>
  void do_apply_kernel(const opT *op, const Tensor<R> &c,
                       const do_op_args<OPDIM> &args) {

    tensorT result =
        op->apply(args.key, args.d, c, args.tol / args.fac / args.cnorm);
    ...
    if (result.normf() > 0.3 * args.tol / args.fac) {
      coeffs.task(args.dest, &nodeT::accumulate2, result, coeffs, args.dest,
                  TaskAttributes::hipri());
      ...
      if (args.dest == world.rank()) {
        ...
        coeffs.task(args.dest, &nodeT::accumulate, result, coeffs, args.dest,
                    TaskAttributes::hipri());
      }
    }
  }

  /// same as do_apply_kernel, but use full rank tensors as input and low rank
  /// tensors as output
  ...
  /// @param[in]  args    laziness holding norm of the coefficients,
  /// displacement, destination, ..
  /// @param[in]  apply_targs TensorArgs with tightened threshold for
  /// accumulation
  /// @return     nothing, but accumulate the result tensor into the destination
  /// node
  template <typename opT, typename R, size_t OPDIM>
  double do_apply_kernel2(const opT *op, const Tensor<R> &c,
                          const do_op_args<OPDIM> &args,
    ...
    if (norm > 0.3 * args.tol / args.fac) {
      ...
      coeffs.task(args.dest, &nodeT::accumulate, result, coeffs, args.dest,
                  apply_targs, TaskAttributes::hipri());
  ...
  /// same as do_apply_kernel2, but use low rank tensors as input and low rank
  /// tensors as output
  ...
  /// node
  template <typename opT, typename R, size_t OPDIM>
  double do_apply_kernel3(const opT *op, const GenTensor<R> &coeff,
                          const do_op_args<OPDIM> &args,
                          const TensorArgs &apply_targs) {

    coeffT result;
    if (2 * OPDIM == NDIM)
      result = op->apply2_lowdim(args.key, args.d, coeff,
                                 args.tol / args.fac / args.cnorm,
                                 args.tol / args.fac);
    if (OPDIM == NDIM)
      result =
          op->apply2(args.key, args.d, coeff, args.tol / args.fac / args.cnorm,
                     args.tol / args.fac);
    ...
    if (result_norm > 0.3 * args.tol / args.fac) {
      ...
      double cpu0 = cpu_time();
      if (not result.is_of_tensortype(targs.tt))
        result = result.convert(targs);
      double cpu1 = cpu_time();
      timer_lr_result.accumulate(cpu1 - cpu0);
  ...
  double vol_nsphere(int n, double R) {
    return std::pow(madness::constants::pi, n * 0.5) * std::pow(R, n) /
           std::tgamma(1 + 0.5 * n);
  }
      ...
      if (node.has_coeff()) {
        if (node.coeff().dim(0) != k || op.doleaves) {
          ProcessID p = FunctionDefaults<NDIM>::get_apply_randomize()
                            ? world.random_proc()
                            : coeffs.owner(key);
          //                        woT::task(p, &implT:: template
          //                        do_apply<opT,R>, &op, key, node.coeff());
          //                        //.full_tensor_copy() ????? why copy ????
          woT::task(p, &implT::template do_apply<opT, R>, &op, key,
                    node.coeff().reconstruct_tensor());
        }
      }
    }
    if (fence)
  ...
  /// invoked by result; the result is accumulated inplace to this's tree at
  /// various FunctionNodes
  /// @param[in] op     the operator to act on the source function
  /// @param[in] key    key of the source FunctionNode of f which is processed
  /// (see "source")
  /// @param[in] coeff  coeffs of FunctionNode being processed
  /// @param[in] do_kernel	true: do the 0-disp only; false: do everything
  /// but the kernel
  /// @return	   max norm, and will modify or include new nodes in this' tree
  template <typename opT, typename R>
  double do_apply_directed_screening(const opT *op, const keyT &key,
                                     const coeffT &coeff,
    ...
    // source is that part of key that corresponds to those dimensions being
    // processed
    const opkeyT source = op->get_source_key(key);
    ...
    // for the kernel it may be more efficient to do the convolution in full
    // rank
    tensorT coeff_full;
    ...
    for (typename std::vector<opkeyT>::const_iterator it = disp.begin();
      ...
      if (do_kernel and (shell > 0))
        break;
      if ((not do_kernel) and (shell == 0))
        continue;

      keyT disp1;
      if (op->particle() == 1)
        disp1 = it->merge_with(nullkey);
      else if (op->particle() == 2)
        disp1 = nullkey.merge_with(*it);
      else {
      ...
      if (not screened) {
        ...
        if (cnorm * opnorm > tol / fac) {
          ...
          double cost_ratio = op->estimate_costs(source, d, coeff_SVD,
                                                 tol / fac / cnorm, tol / fac);
          ...
          if (cost_ratio > 0.0) {
            ...
            norm = 0.0;
            if (cost_ratio < 1.0) {
              if (not coeff_full.has_data())
                coeff_full = coeff.full_tensor_copy();
  ...
  void apply_source_driven(opT &op, const FunctionImpl<R, NDIM> &f,
    ...
    for (typename dcT::const_iterator it = f.get_coeffs().begin(); it != end;
         ++it) {
      ...
      if (coeff.has_data() and (coeff.rank() != 0)) {
        ProcessID p = FunctionDefaults<NDIM>::get_apply_randomize()
                          ? world.random_proc()
                          : coeffs.owner(key);
        woT::task(p, &implT::template do_apply_directed_screening<opT, R>, &op,
                  key, coeff, true);
        woT::task(p, &implT::template do_apply_directed_screening<opT, R>, &op,
                  key, coeff, false);
      }
  ...
  /// invoked by result
  /// here we use the fact that the hi-dim NS coefficients on all scales are
  /// exactly the outer product of the underlying low-dim functions (also in NS
  /// form), so we don't need to construct the full hi-dim tree and then turn it
  /// into NS form.
  ...
  template <typename opT, std::size_t LDIM> struct recursive_apply_op {
    bool randomize() const { return true; }
    ...
    // ctor
    recursive_apply_op() {}
    recursive_apply_op(implT *result, const CoeffTracker<T, LDIM> &iaf,
                       const CoeffTracker<T, LDIM> &iag, const opT *apply_op)
        : result(result), iaf(iaf), iag(iag), apply_op(apply_op) {
      MADNESS_ASSERT(LDIM + LDIM == NDIM);
    }
    recursive_apply_op(const recursive_apply_op &other)
        : result(other.result), iaf(other.iaf), iag(other.iag),
          apply_op(other.apply_op) {}
    ...
    std::pair<bool, coeffT> operator()(const Key<NDIM> &key) const {
      ...
      if (not is_leaf) {
        ...
        double norm0 = result->do_apply_directed_screening<opT, T>(
    ...
    /// sole purpose is to wait for the kernel norm, wrap it and send it back to
    /// caller
    ...
    this_type make_child(const keyT &child) const {
      ...
      return this_type(result, iaf.make_child(key1), iag.make_child(key2),
                       apply_op);
    }
    ...
    Future<this_type> activate() const {
      ...
      return result->world.taskq.add(
          detail::wrap_mem_fn(*const_cast<this_type *>(this),
                              &this_type::forward_ctor),
          result, f1, g1, apply_op);
    }

    this_type forward_ctor(implT *r, const CoeffTracker<T, LDIM> &f1,
  ...
  /// @param[in]	rimpl    a dummy function for recursive_op to insert
  /// data
  template <typename opT>
  void recursive_apply(opT &apply_op, const implT *fimpl, implT *rimpl,
                       const bool fence) {
  ...
  /// recursive part of recursive_apply
  template <typename opT> struct recursive_apply_op2 {
    bool randomize() const { return true; }
    ...
    recursive_apply_op2(const recursive_apply_op2 &other)
        : result(other.result), iaf(other.iaf), apply_op(other.apply_op) {}
    ...
    Future<this_type> activate() const {
      ...
      //                Future<ctL> g1=g.activate();
      //                return
      //                h->world.taskq.add(detail::wrap_mem_fn(*const_cast<this_type
      //                *> (this),
    ...
    /// taskq-compatible ctor
    this_type forward_ctor(implT *result1, const ctT &iaf1,
                           const opT *apply_op1) {
  ...
  /// this assumption
  template <typename opT>
  double err_box(const keyT &key, const nodeT &node, const opT &func, int npt,
                 const Tensor<double> &qx, const Tensor<double> &quad_phit,
  ...
  template <typename opT> class do_err_box {
    const implT *impl;
    const opT *func;
    int npt;
    Tensor<double> qx;
  ...
  public:
    ...
    do_err_box(const implT *impl, const opT *func, int npt,
               const Tensor<double> &qx, const Tensor<double> &quad_phit,
               const Tensor<double> &quad_phiw)
        : impl(impl), func(func), npt(npt), qx(qx), quad_phit(quad_phit),
          quad_phiw(quad_phiw) {}

    do_err_box(const do_err_box &e)
        : impl(e.impl), func(e.func), npt(e.npt), qx(e.qx),
          quad_phit(e.quad_phit), quad_phiw(e.quad_phiw) {}
  ...
  template <typename opT> double errsq_local(const opT &func) const {
    PROFILE_MEMBER_FUNC(FunctionImpl);
    // Make quadrature rule of higher order
    const int npt = cdata.npt + 1;
    Tensor<double> qx, qw, quad_phi, quad_phiw, quad_phit;
    FunctionCommonData<T, NDIM>::_init_quadrature(k + 1, npt, qx, qw, quad_phi,
                                                  quad_phiw, quad_phit);

    typedef Range<typename dcT::const_iterator> rangeT;
    rangeT range(coeffs.begin(), coeffs.end());
    return world.taskq.reduce<double, rangeT, do_err_box<opT>>(
        range, do_err_box<opT>(this, &func, npt, qx, quad_phit, quad_phiw));
  }
  ...
  template <typename R> struct do_inner_local {
    ...
    resultT operator()(typename dcT::const_iterator &it) const {
      ...
      if (fnode.has_coeff()) {
        if (other->coeffs.probe(it->first)) {
          const FunctionNode<R, NDIM> &gnode =
              other->coeffs.find(key).get()->second;
          if (gnode.has_coeff()) {
            if (gnode.coeff().dim(0) != fnode.coeff().dim(0)) {
              madness::print("INNER", it->first, gnode.coeff().dim(0),
                             fnode.coeff().dim(0));
              MADNESS_EXCEPTION(
                  "functions have different k or compress/reconstruct error",
                  0);
            }
            if (leaves_only) {
  ...
  inner_local(const FunctionImpl<R, NDIM> &g) const {
    PROFILE_MEMBER_FUNC(FunctionImpl);
    typedef Range<typename dcT::const_iterator> rangeT;
    typedef TENSOR_RESULT_TYPE(T, R) resultT;
    ...
    return world.taskq.reduce<resultT, rangeT, do_inner_local<R>>(
        ...
        do_inner_local<R>(&g, leaves_only));
  }

  /// compute the inner product of this range with other
  template <typename R> struct do_inner_local_on_demand {
    const FunctionImpl<T, NDIM> *bra;
    const FunctionImpl<R, NDIM> *ket;
    bool leaves_only = true;
    typedef TENSOR_RESULT_TYPE(T, R) resultT;

    do_inner_local_on_demand(const FunctionImpl<T, NDIM> *bra,
                             const FunctionImpl<R, NDIM> *ket,
    ...
    resultT operator()(typename dcT::const_iterator &it) const {
      ...
      const keyT &key = it->first;
      const nodeT &fnode = it->second;
      if (not fnode.has_coeff())
        return resultT(0.0); // probably internal nodes

      // assuming all boxes (esp the low-dim ones) are local, i.e. the functions
      // are replicated
      auto find_valid_parent = [](auto &key, auto &impl,
                                  auto &&find_valid_parent) {
        MADNESS_CHECK(impl->get_coeffs().owner(key) ==
                      impl->world.rank()); // make sure everything is local!
        if (impl->get_coeffs().probe(key))
          return key;
      ...
      auto get_coeff = [&find_valid_parent](const auto &key,
                                            const auto &v_impl) {
        if ((v_impl.size() > 0) and v_impl.front().get()) {
          ...
          //                        typedef typename
          //                        std::decay_t<decltype(v_impl)>::value_type
          //                        S;
          return GenTensor<S>();
          //                        return GenTensor<typename
          //                        std::decay_t<decltype(*impl)>::typeT>();
        }
      };
      ...
      auto func = dynamic_cast<CompositeFunctorInterface<R, NDIM, LDIM> *>(
          ket->functor.get());
      MADNESS_ASSERT(func);

      MADNESS_CHECK_THROW(func->impl_ket_vector.size() == 0 or
                              func->impl_ket_vector.size() == 1,
                          "only one ket function supported in inner_on_demand");
      MADNESS_CHECK_THROW(func->impl_p1_vector.size() == 0 or
                              func->impl_p1_vector.size() == 1,
                          "only one p1 function supported in inner_on_demand");
      MADNESS_CHECK_THROW(func->impl_p2_vector.size() == 0 or
                              func->impl_p2_vector.size() == 1,
      ...
      if (coeff_ket.has_data() and coeff_p1.has_data()) {
        ...
        coeff_ket = pm(
            key,
            outer(coeff_p1, coeff_p2, TensorArgs(TT_FULL, -1.0)).full_tensor());
        error += pm.error;
      } else if (coeff_ket.has_data() or coeff_p1.has_data()) {
        coeff_ket =
            (coeff_ket.has_data()) ? coeff_ket : outer(coeff_p1, coeff_p2);
      ...
      resultT result;
      if (func->impl_eri) { // project bra*ket onto eri, avoid multiplication
                            // with eri
        ...
        error += pm.error;
        if (error > 1.e-3)
          print("error in key", key, error);
        result = coeff_eri.full_tensor().trace(braket);

      } else { // no eri, project ket onto bra
        result =
            coeff_bra.full_tensor_copy().trace_conj(v1v2ket.full_tensor_copy());
      }
      return result;
    }
  ...
  template <typename R>
  TENSOR_RESULT_TYPE(T, R)
  inner_local_on_demand(const FunctionImpl<R, NDIM> &gimpl) const {
    PROFILE_MEMBER_FUNC(FunctionImpl);
    MADNESS_CHECK(this->is_reconstructed());

    typedef Range<typename dcT::const_iterator> rangeT;
    rangeT range(coeffs.begin(), coeffs.end());
    return world.taskq.reduce<T, rangeT, do_inner_local_on_demand<T>>(
        range, do_inner_local_on_demand<R>(this, &gimpl));
  }
  ...
  /// Returns map of union of local keys to vector of indexes of functions
  /// containing that key
  ...
  make_key_vec_map(const std::vector<const FunctionImpl<T, NDIM> *> &v) {
    ...
    for (unsigned int i = 0; i < v.size(); i++) {
      ...
      v[i]->world.taskq.add(*(v[i]), &FunctionImpl<T, NDIM>::add_keys_to_map,
                            &map, int(i));
    }
    if (v.size())
    ...
    for (typename mapT::iterator lit = lstart; lit != lend; ++lit) {
      ...
      if (rit != rmap_ptr->end()) {
        ...
        for (unsigned int iv = 0; iv < nleft; ++iv)
          Left(iv, _) = *(leftv[iv].second);
        for (unsigned int jv = 0; jv < nright; ++jv)
          Right(jv, _) = *(rightv[jv].second);
        // call mxmT from mxm.h in tensor
        if (TensorTypeData<T>::iscomplex)
          Left = Left.conj(); // Should handle complex case and leave real case
                              // alone
  ...
  static double conj(float x) { return x; }
  ...
  inner_local(const std::vector<const FunctionImpl<T, NDIM> *> &left,
    ...
    typename FunctionImpl<R, NDIM>::mapT *rmap_ptr =
        (typename FunctionImpl<R, NDIM>::mapT *)(&lmap);
    ...
    while (lstart != lmap.end()) {
      ...
      advance(lend, chunk);
      left[0]->world.taskq.add(&FunctionImpl<T, NDIM>::do_inner_localX<R>,
                               lstart, lend, rmap_ptr, sym, &r, &mutex);
      lstart = lend;
    }
  ...
  template <typename R> void print_type_in_compilation_error(R &&) {
    static_assert(
        !std::is_same<R, int>::value && std::is_same<R, int>::value,
  ...
  void partial_inner(const FunctionImpl<Q, LDIM> &g,
    ...
    for (std::size_t n = 0; n < nmax; ++n) {
      ...
      if ((g_ijlist.size() == 0) and (h_ijlist.size() == 0))
        break;
      // double wall1 = wall_time();
      // wall_get_lists += (wall1 - wall0);
      // wall0 = wall1;
      //                print("g_jlist");
      //                for (const auto& kv : g_jlist)
      //                print(kv.first,kv.second); print("h_jlist"); for (const
      //                auto& kv : h_jlist) print(kv.first,kv.second);

      // next lines will insert s nodes into g and h -> possible race condition!
      bool this_first = true; // are the remaining indices of g before those of
                              // g: f(x,z) = g(x,y) h(y,z)
      // CDIM, NDIM, KDIM
      contractionmapT contraction_map = g_nc.recur_down_for_contraction_map(
          g_nc.key0(), g_nc.get_coeffs().find(g_nc.key0()).get()->second, v1,
          v2, h_ijlist, h_jlist, this_first, thresh);
      ...
      contractionmapT contraction_map1 = h_nc.recur_down_for_contraction_map(
          h_nc.key0(), hnode0, v2, v1, g_ijlist, g_jlist, this_first, thresh);
    ...
    for (const auto &contraction_map : all_contraction_maps) {
      for (const auto &key_list : contraction_map) {
        ...
        woT::task(coeffs.owner(key),
                  &implT::template partial_inner_contract<Q, LDIM, R, KDIM>, &g,
                  &h, v1, v2, key, list);
      }
    }
  }
  ...
  /// find all nodes with d coefficients and return a list of complete keys and
  /// of keys holding only the y dimension, also the maximum norm of all d for
  /// the j dimension
  /// @param[in]  n   the scale
  /// @param[in]  v   array holding the indices of the integration variable
  /// @return     ijlist: list of all nodes with d coeffs; jlist: j-part of ij
  /// list only
  template <std::size_t CDIM>
  std::tuple<std::set<Key<NDIM>>, std::map<Key<CDIM>, double>>
  get_contraction_node_lists(const std::size_t n,
                             const std::array<int, CDIM> &v) const {

    const auto &cdata = get_cdata();
    auto has_d_coeffs = [&cdata](const coeffT &coeff) {
      if (coeff.has_no_data())
        return false;
    ...
    std::map<Key<CDIM>, double>
        j_list; // only that dimension that will be contracted
  ...
  /// coefficients if necessary. Make list of nodes n − ijk as map(n-ik,
  /// list(j)).
  ///
  /// !! WILL ADD NEW S NODES TO THIS TREE THAT MUST BE REMOVED TO AVOID
  ...
  /// @param[in]  ij_other_list  list of nodes of the other function that will
  /// be contracted (and their parents)
  /// @param[in]  j_other_list  list of column nodes of the other function that
  /// will be contracted (and their parents)
  /// @param[in]  max_d_norm  max d coeff norm of the nodes in j_list
  /// @param[in]  this_first  are the remaining coeffs of this functions first
  /// or last in the result function
  /// @param[in]  thresh  threshold for including nodes in the contraction:
  /// snorm*dnorm > thresh
  ...
  std::multimap<Key<FDIM>, std::list<Key<CDIM>>> recur_down_for_contraction_map(
    ...
    const bool sd_norm_product_large =
        node.get_snorm() * max_d_norm > truncate_tol(thresh, key);
    //            print("sd_product_norm",node.get_snorm() * max_d_norm,
    //            thresh);
    ...
    if (final_scale and sd_norm_product_large) {
      for (auto &other_key : ij_other_list) {
        ...
        Key<FDIM> ik_key =
            (this_first) ? i_key.merge_with(k_key) : k_key.merge_with(i_key);
        //                    print("ik_key",ik_key);
        //                    MADNESS_CHECK(contraction_map.count(ik_key)==0);
        contraction_map.insert(
            std::make_pair(ik_key, std::list<Key<CDIM>>{j_this_key}));
      }
      return contraction_map;
    }
    ...
    if (continue_recursion) {
      ...
      for (KeyChildIterator<NDIM> kit(key); kit; ++kit) {
        ...
        bool need_s_coeffs =
            childnode_exists ? (acc->second.get_snorm() <= 0.0) : true;
        ...
        if (need_s_coeffs)
          childnode.recompute_snorm_and_dnorm(get_cdata());
        //                    print("recurring down to",child);
        contraction_map.merge(recur_down_for_contraction_map(
            child, childnode, v_this, v_other, ij_other_list, j_other_list,
            this_first, thresh));
  ...
  /// @param[in]  j_key_list list of contraction index-j keys contributing to
  /// this' node
  template <typename Q, std::size_t LDIM, typename R, std::size_t KDIM,
            std::size_t CDIM = (KDIM + LDIM - NDIM) / 2>
  void partial_inner_contract(const FunctionImpl<Q, LDIM> *g,
                              const FunctionImpl<R, KDIM> *h,
                              const std::array<int, CDIM> v1,
                              const std::array<int, CDIM> v2,
                              const Key<NDIM> &key,
    ...
    for (const auto &j_key : j_key_list) {
      ...
      auto v_complement = [](const auto &v, const auto &vc) {
        constexpr std::size_t VDIM =
            std::tuple_size<std::decay_t<decltype(v)>>::value;
        constexpr std::size_t VCDIM =
            std::tuple_size<std::decay_t<decltype(vc)>>::value;
        std::array<int, VCDIM> result;
        for (std::size_t i = 0; i < VCDIM; i++)
          result[i] = (v.back() + i + 1) % (VDIM + VCDIM);
        return result;
      };
      auto make_ij_key = [&v_complement](const auto i_key, const auto j_key,
                                         const auto &v) {
      ...
      // offset: 0 for full tensor, 1 for svd representation with rand being the
      // first dimension (r,d1,d2,d3) -> (r,d1*d2*d3)
      auto fuse = [](Tensor<T> tensor, const std::array<int, CDIM> &v,
                     int offset) {
        for (std::size_t i = 0; i < CDIM - 1; ++i) {
          MADNESS_CHECK((v[i] + 1) ==
                        v[i + 1]); // make sure v is contiguous and ascending
          tensor = tensor.fusedim(v[0] + offset);
        }
        return tensor;
      };

      // use case: partial_projection of 2-electron functions in svd
      // representation  f(1) = \int g(2) h(1,2) d2 c_i = \sum_j a_j b_ij =
      // \sum_jr a_j b_rj b'_rj
      //                       = \sum_jr ( a_j b_rj) b'_rj )
      auto contract2 = [](const auto &svdcoeff, const auto &tensor,
                          const int particle) {
#if HAVE_GENTENSOR
        const int spectator_particle = (particle + 1) % 2;
        Tensor<Q> gtensor =
            svdcoeff.get_svdtensor().make_vector_with_weights(particle);
        gtensor =
            gtensor.reshape(svdcoeff.rank(), gtensor.size() / svdcoeff.rank());
        MADNESS_CHECK(gtensor.ndim() == 2);
        Tensor<Q> gtensor_other =
            svdcoeff.get_svdtensor().ref_vector(spectator_particle);
        Tensor<T> tmp1 = inner(gtensor, tensor.flat(), 1,
                               0); // tmp1(r) = sum_j a'_(r,j) b(j)
        MADNESS_CHECK(tmp1.ndim() == 1);
        Tensor<T> tmp2 =
            inner(gtensor_other, tmp1, 0, 0); // tmp2(i) = sum_r a_(r,i) tmp1(r)
        return tmp2;
#else
      ...
      if (gcoeff.is_full_tensor() and hcoeff.is_full_tensor() and
          result_coeff.is_full_tensor()) {
        ...
        if (key.level() > 0) {
          ...
          result_coeff.full_tensor()(get_cdata().s0) -=
              inner(gtensor, htensor, v1[0], v2[0]);
        }
      }

      // use case: 2-electron functions in svd representation  f(1,3) = \int
      // g(1,2) h(2,3) d2 c_ik = \sum_j a_ij b_jk = \sum_jrr' a_ri a'_rj b_r'j
      // b_r'k
      //                         = \sum_jrr' ( a_ri (a'_rj b_r'j) )  b_r'k
      //                         = \sum_jrr' c_r'i  b_r'k
      else if (gcoeff.is_svd_tensor() and hcoeff.is_svd_tensor() and
               result_coeff.is_svd_tensor()) {
        ...
        // merge multiple contraction dimensions into one
        Tensor<Q> gtensor =
            gcoeff1.get_svdtensor().flat_vector_with_weights(gparticle);
        Tensor<Q> gtensor_other =
            gcoeff1.get_svdtensor().flat_vector((gparticle + 1) % 2);
        Tensor<R> htensor =
            hcoeff1.get_svdtensor().flat_vector_with_weights(hparticle);
        Tensor<R> htensor_other =
            hcoeff1.get_svdtensor().flat_vector((hparticle + 1) % 2);
        Tensor<T> tmp1 =
            inner(gtensor, htensor, 1, 1); // tmp1(r,r') = sum_j b(r,j) a(r',j)
        Tensor<T> tmp2 = inner(tmp1, gtensor_other, 0,
                               0); // tmp2(r',i) = sum_r tmp1(r,r') a(r,i)
        ...
        result_tmp.get_svdtensor().set_vectors_and_weights(w, tmp2,
                                                           htensor_other);
        if (key.level() > 0) {
          GenTensor<Q> gcoeff2 = copy(gcoeff1(g->get_cdata().s0));
          GenTensor<R> hcoeff2 = copy(hcoeff1(h->get_cdata().s0));
          Tensor<Q> gtensor =
              gcoeff2.get_svdtensor().flat_vector_with_weights(gparticle);
          Tensor<Q> gtensor_other =
              gcoeff2.get_svdtensor().flat_vector((gparticle + 1) % 2);
          Tensor<R> htensor =
              hcoeff2.get_svdtensor().flat_vector_with_weights(hparticle);
          Tensor<R> htensor_other =
              hcoeff2.get_svdtensor().flat_vector((hparticle + 1) % 2);
          Tensor<T> tmp1 = inner(gtensor, htensor, 1,
                                 1); // tmp1(r,r') = sum_j b(r,j) a(r',j)
          Tensor<T> tmp2 = inner(tmp1, gtensor_other, 0,
                                 0); // tmp2(r',i) = sum_r tmp1(r,r') a(r,i)
          Tensor<typename Tensor<T>::scalar_type> w(tmp2.dim(0));
          MADNESS_CHECK(tmp2.dim(0) == htensor_other.dim(0));
          ...
          result_coeff1.get_svdtensor().set_vectors_and_weights(w, tmp2,
                                                                htensor_other);
      ...
      // \sum_jr a_j b_rj b'_rj
      //                       = \sum_jr ( a_j b_rj) b'_rj )
      else if (gcoeff.is_full_tensor() and hcoeff.is_svd_tensor() and
               result_coeff.is_full_tensor()) {
        ...
        if (key.level() > 0)
          r(get_cdata().s0) -= contract2(
              copy(hcoeff1(h->get_cdata().s0)),
              copy(gcoeff.full_tensor()(g->get_cdata().s0)), hparticle);
        result_coeff.full_tensor() += r;
      }
      // use case: partial_projection of 2-electron functions in svd
      // representation  f(1) = \int g(1,2) h(2) d2 c_i = \sum_j a_ij b_j =
      // \sum_jr a_ri a'_rj b_j
      //                       = \sum_jr ( a_ri (a'_rj b_j) )
      else if (gcoeff.is_svd_tensor() and hcoeff.is_full_tensor() and
               result_coeff.is_full_tensor()) {
        MADNESS_CHECK(v1[0] == 0 or v1[CDIM - 1] == LDIM - 1);
        ...
        if (key.level() > 0)
          r(get_cdata().s0) -= contract2(
              copy(gcoeff1(g->get_cdata().s0)),
              copy(hcoeff.full_tensor()(h->get_cdata().s0)), gparticle);
        result_coeff.full_tensor() += r;
    ...
    if (coeffs.is_local(key))
      coeffs.send(key, &nodeT::accumulate, result_coeff, coeffs, key,
                  get_tensor_args());
    else
      coeffs.task(key, &nodeT::accumulate, result_coeff, coeffs, key,
                  get_tensor_args(), TaskAttributes::hipri());
  }

  /// Return the inner product with an external function on a specified function
  /// node.

  /// @param[in] key Key of the function node to compute the inner product on.
  /// (the domain of integration)
  /// @param[in] c Tensor of coefficients for the function at the function node
  /// given by key
  /// @param[in] f Reference to FunctionFunctorInterface. This is the externally
  /// provided function
  /// @return Returns the inner product over the domain of a single function
  /// node, no guarantee of accuracy.
  T inner_ext_node(
      keyT key, tensorT c,
      const std::shared_ptr<FunctionFunctorInterface<T, NDIM>> f) const {
  ...
  /// @param[in] key Key of the function node on which to compute inner product
  /// (the domain of integration)
  ...
  T inner_ext_recursive(
    ...
    if (coeffs.find(key).get()->second.has_children()) {
      ...
      for (KeyChildIterator<NDIM> it(key); it; ++it, ++i) {
        ...
        tensorT cc =
            coeffs.find(child).get()->second.coeff().full_tensor_copy();
    ...
    if (std::abs(new_inner - old_inner) <= thresh) {
      ...
      for (KeyChildIterator<NDIM> it(key); it; ++it, ++i) {
        ...
        tensorT cc = tensorT(c_child(child_patch(child)));
        result +=
            inner_ext_recursive(child, cc, f, leaf_refine, inner_child(i));
      }
    }
  ...
  struct do_inner_ext_local_ffi {
    ...
    do_inner_ext_local_ffi(
        ...
        : fref(f), impl(impl), leaf_refine(leaf_refine),
          do_leaves(do_leaves) {};

    T operator()(typename dcT::const_iterator &it) const {
      if (do_leaves and it->second.is_leaf()) {
        tensorT cc = it->second.coeff().full_tensor();
        return impl->inner_adaptive_recursive(it->first, cc, fref, leaf_refine,
                                              T(0));
      } else if ((not do_leaves) and
                 (it->first.level() == impl->initial_level)) {
        tensorT cc = it->second.coeff().full_tensor();
        return impl->inner_ext_recursive(it->first, cc, fref, leaf_refine,
                                         T(0));
      } else {
        return 0.0;
      }
  ...
  T inner_ext_local(const std::shared_ptr<FunctionFunctorInterface<T, NDIM>> f,
                    const bool leaf_refine) const {
    typedef Range<typename dcT::const_iterator> rangeT;

    return world.taskq.reduce<T, rangeT, do_inner_ext_local_ffi>(
        rangeT(coeffs.begin(), coeffs.end()),
  ...
  /// @return Returns the inner product over the domain of a single function,
  /// checks for convergence.
  T inner_adaptive_recursive(
      keyT key, const tensorT &c,
      const std::shared_ptr<FunctionFunctorInterface<T, NDIM>> f,
      const bool leaf_refine, T old_inner = T(0)) const {
  ...
  template <typename L>
  tensorT gaxpy_ext_node(keyT key, Tensor<L> lc, T (*f)(const coordT &),
  ...
  void gaxpy_ext_recursive(const keyT &key, const FunctionImpl<L, NDIM> *left,
    ...
    if (not below_leaf) {
      ...
      if (left_leaf) {
        ...
        for (KeyChildIterator<NDIM> it(key); it; ++it) {
          const keyT &child = it.key();
          woT::task(left->coeffs.owner(child),
                    &implT::template gaxpy_ext_recursive<L>, child, left,
                    Tensor<L>(), tensorT(), f, alpha, beta, tol, below_leaf);
        }
        return;
      }
    }
    ...
    for (KeyChildIterator<NDIM> it(key); it; ++it) {
      ...
      c_child(child_patch(child)) =
          gaxpy_ext_node(child, lcoeff, f, alpha, beta);
    }
    ...
    if (dnorm <= truncate_tol(tol, key)) {
      ...
      for (KeyChildIterator<NDIM> it(key); it; ++it) {
        ...
        woT::task(left->coeffs.owner(child),
                  &implT::template gaxpy_ext_recursive<L>, child, left,
                  left_coeff, child_coeff, f, alpha, beta, tol, below_leaf);
      }
    }
  }

  template <typename L>
  void gaxpy_ext(const FunctionImpl<L, NDIM> *left, T (*f)(const coordT &),
                 T alpha, T beta, double tol, bool fence) {
    if (world.rank() == coeffs.owner(cdata.key0))
      gaxpy_ext_recursive<L>(cdata.key0, left, Tensor<L>(), tensorT(), f, alpha,
                             beta, tol, false);
    if (fence)
      world.gop.fence();
  }
  ...
  /// @param[in]  result  lo-dim function of NDIM-LDIM \todo Should this be
  /// param[out]?
  /// @param[in]  gimpl  	lo-dim function of LDIM
  /// @param[in]  dim over which dimensions to be integrated: 0..LDIM or
  /// LDIM..LDIM+NDIM-1
  template <size_t LDIM>
  void project_out(FunctionImpl<T, NDIM - LDIM> *result,
                   const FunctionImpl<T, LDIM> *gimpl, const int dim,
                   const bool fence) {
  ...
  /// project the low-dim function g on the hi-dim function f: result(x) =
  /// <f(x,y) | g(y)>
  template <size_t LDIM> struct project_out_op {
    bool randomize() const { return false; }
    ...
    project_out_op(const project_out_op &other)
        : fimpl(other.fimpl), result(other.result), iag(other.iag),
          dim(other.dim) {}
    ...
    Future<argT> operator()(const Key<NDIM> &key) const {
      ...
      if (fcoeff.is_full_tensor()) {
        ...
        if (fcoeff.rank() > 0) {
          ...
          const tensorT atensor =
              fcoeff.get_svdtensor().flat_vector(dim); // a_rj
      ...
      result->coeffs.task(dest, &FunctionNode<T, LDIM>::accumulate2, final,
                          result->coeffs, dest, TaskAttributes::hipri());
    ...
    /// taskq-compatible ctor
    this_type forward_ctor(const implT *fimpl1, implL1 *result1,
                           const ctL &iag1, const int dim1) {
  ...
  /// project the low-dim function g on the hi-dim function f: this(x) = <f(x,y)
  /// | g(y)>
  ...
  /// LDIM..LDIM+NDIM-1
  template <size_t LDIM>
  void project_out2(const FunctionImpl<T, LDIM + NDIM> *f,
                    const FunctionImpl<T, LDIM> *g, const int dim) {

    typedef std::pair<keyT, coeffT> pairT;
    typedef
        typename FunctionImpl<T, NDIM + LDIM>::dcT::const_iterator fiterator;
    ...
    for (fiterator it = f->get_coeffs().begin(); it != end; ++it) {
      ...
      if (fnode.is_leaf() and fcoeff.has_data()) {
        ...
        if (dim == 0) {
          ...
          Future<pairT> result;
          //                        sock_it_to_me(key1,
          //                        result.remote_ref(world));
          g->task(coeffs.owner(key1), &implT::sock_it_to_me, key1,
                  result.remote_ref(world), TaskAttributes::hipri());
          woT::task(world.rank(), &implT::template do_project_out<LDIM>, fcoeff,
                    result, key1, key2, dim);
          ...
          //                        sock_it_to_me(key2,
          //                        result.remote_ref(world));
          g->task(coeffs.owner(key2), &implT::sock_it_to_me, key2,
                  result.remote_ref(world), TaskAttributes::hipri());
          woT::task(world.rank(), &implT::template do_project_out<LDIM>, fcoeff,
                    result, key2, key1, dim);
  ...
  /// compute the inner product of two nodes of only some dimensions and
  /// accumulate on result

  /// invoked by result
  /// @param[in]  fcoeff  coefficients of high dimension LDIM+NDIM
  /// @param[in]  gpair   key and coeffs of low dimension LDIM (possibly a
  /// parent node)
  /// @param[in]  gkey    key of actual low dim node (possibly the same as
  /// gpair.first, iff gnode exists)
  /// @param[in]  dest    destination node for the result
  /// @param[in]  dim     which dimensions should be contracted: 0..LDIM-1 or
  /// LDIM..NDIM+LDIM-1
  template <size_t LDIM>
  void do_project_out(const coeffT &fcoeff, const std::pair<keyT, coeffT> gpair,
                      const keyT &gkey, const Key<NDIM> &dest,
                      const int dim) const {
    ...
    // let's specialize for the time being on SVD tensors for f and full tensors
    // of half dim for g
    ...
    for (int r = 0; r < fcoeff.rank(); ++r) {
      s[0] = Slice(r, r);
      const tensorT contracted_tensor =
          fcoeff.config().ref_vector(dim)(s).reshape(k, k, k);
      const tensorT other_tensor =
          fcoeff.config().ref_vector(otherdim)(s).reshape(k, k, k);
  ...
  /// Returns the maximum depth of the tree ... collective ... global
  /// sum/broadcast
  ...
  /// Returns the size of the tree structure of the function ... collective
  /// global sum
  ...
  /// \return The hash.
  inline friend hashT
  hash_value(const std::shared_ptr<FunctionImpl<T, NDIM>> impl) {
...
struct ArchiveLoadImpl<Archive, const FunctionImpl<T, NDIM> *> {
  static void load(const Archive &ar, const FunctionImpl<T, NDIM> *&ptr) {
    ...
    if (exists) {
      ...
      if (!ptr_opt)
        MADNESS_EXCEPTION("FunctionImpl: remote operation attempting to use a "
                          "locally uninitialized object",
                          0);
      ptr = static_cast<const FunctionImpl<T, NDIM> *>(*ptr_opt);
      if (!ptr)
        MADNESS_EXCEPTION("FunctionImpl: remote operation attempting to use an "
                          "unregistered object",
                          0);
    } else {
      ptr = nullptr;
...
struct ArchiveStoreImpl<Archive, const FunctionImpl<T, NDIM> *> {
  static void store(const Archive &ar,
                    const FunctionImpl<T, NDIM> *const &ptr) {
...
struct ArchiveLoadImpl<Archive, std::shared_ptr<const FunctionImpl<T, NDIM>>> {
  static void load(const Archive &ar,
                   std::shared_ptr<const FunctionImpl<T, NDIM>> &ptr) {
...
struct ArchiveStoreImpl<Archive, std::shared_ptr<const FunctionImpl<T, NDIM>>> {
  static void store(const Archive &ar,
                    const std::shared_ptr<const FunctionImpl<T, NDIM>> &ptr) {
    ArchiveStoreImpl<Archive, const FunctionImpl<T, NDIM> *>::store(ar,
                                                                    ptr.get());
  }
};
...
struct ArchiveLoadImpl<Archive, std::shared_ptr<FunctionImpl<T, NDIM>>> {
  static void load(const Archive &ar,
                   std::shared_ptr<FunctionImpl<T, NDIM>> &ptr) {
...
struct ArchiveStoreImpl<Archive, std::shared_ptr<FunctionImpl<T, NDIM>>> {
  static void store(const Archive &ar,
                    const std::shared_ptr<FunctionImpl<T, NDIM>> &ptr) {

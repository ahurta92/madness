...
void ExcitedResponse::initialize(World &world) {
  ...
  if (r_params.random()) {
    ...
    if (temp_trial.num_states() >= 2 * r_params.num_states()) {
      std::copy(temp_trial.x.begin(),
                temp_trial.x.begin() + 2 * r_params.num_states(),
                trial.x.begin());
    } else if (temp_trial.num_states() >= r_params.num_states()) {
      trial = X_space(world, temp_trial.num_states(), r_params.num_orbitals());
      std::copy(temp_trial.x.begin(),
                temp_trial.x.begin() + temp_trial.num_states(),
                trial.x.begin());
    } else {
      MADNESS_EXCEPTION(
          "guess virtual ao did not produce enough states for calculation", 1);
    }
  }
  ...
  // truncate before load_balancing after projecting
  trial.x.truncate_rf();
  ...
  if (world.size() > 1) {
    ...
    lb.print_cost_per_rank("BEFORE", FunctionDefaults<3>::get_pmap());
    auto newmap = lb.load_balance(2);
    lb.print_cost_per_rank("AFTER", newmap);
    FunctionDefaults<3>::redistribute(world, newmap);
...
void ExcitedResponse::iterate_trial(World &world, X_space &guesses) {
  ...
  while (iteration < r_params.guess_max_iter()) {
    ...
    // Debugging output
    auto [new_omega, rotated_chi, rotated_lambda, rotated_v_x,
          rotated_gamma_x] = rotate_excited_space(world, guesses, temp_Lambda_X,
                                                  temp_V0X, temp_gamma);
    ...
    if (iteration + 1 < r_params.guess_max_iter()) {
      ...
      X_space theta_X =
          X_space(world, rotated_chi.num_states(), rotated_chi.num_orbitals());
...
ExcitedResponse::rotate_excited_space(World &world, X_space &chi, X_space &lchi,
                                      X_space &v_chi, X_space &gamma_chi) {
  // Debugging output
  Tensor<double> A;
  X_space chi_copy = chi.copy();
...
ExcitedResponse::reduce_subspace(World &world, Tensor<double> &S,
                                 Tensor<double> &A,
...
ExcitedResponse::excited_eig(World &world, Tensor<double> &S, Tensor<double> &A,
  ...
  if (r_params.print_level() >= 1) {
    molresponse::end_timer(world, "reduce subspace", "subspace_reduce",
                           iter_timing);
  }
  if (r_params.print_level() >= 1)
    molresponse::start_timer(world);
  ...
  // End timer
  if (r_params.print_level() >= 1) {
    molresponse::end_timer(world, "diagonalize response matrix",
                           "diagonalize_response_matrix", iter_timing);
  }
...
ExcitedResponse::rotate_excited_vectors(World &world, const Tensor<double> &U,
                                        const X_space &chi,
                                        const X_space &l_chi,
...
ExcitedResponse::create_bsh_operators(World &world, const Tensor<double> &shift,
  ...
  // Capture mu_{k,p} = sqrt(-2(epsilon_p + omega_k + shift_{k,p})) for a
  // tabular dump after the loop. Replaces the prior per-(state,orbital)
  // print which produced m*n lines per call (and create_bsh_operators is
  // called many times per outer iter).
  Tensor<double> mu_matrix(static_cast<int>(m), static_cast<int>(n));
  ...
  for (size_t k = 0; k < m; k++) {
    ...
    for (size_t p = 0; p < n; p++) {
      ...
      mu_matrix(static_cast<int>(k), static_cast<int>(p)) = mu;
  ...
  if (world.rank() == 0 && r_params.print_level() >= 2) {
    ...
    for (size_t k = 0; k < m; k++) {
      printf("  state %-2zu ", k);
      for (size_t p = 0; p < n; p++)
        printf("  %8.4f", mu_matrix(static_cast<int>(k), static_cast<int>(p)));
      printf("\n");
    }
    printf("============================================================="
           "=====\n");
  ...
  for (iter = 0; iter < r_params.maxiter(); ++iter) {
    ...
    if (iter > 0) {
      ...
      if (all_done || iter == r_params.maxiter() - 1) {
        ...
        //                if (r_params.plot()) {
        //                    do_vtk_plots(world, 200, r_params.L(), molecule,
        //                    rho0, rho_omega,
        //                                 ground_orbitals, Chi);
        //                }
        break;
    ...
    // residual is computed as new_chi-old_chi where both have been previously
    // rotated.
    auto [new_omega, old_chi, new_chi, new_res] = update_response(
        world, Chi, xc, projector, kain_x_space, x_vectors, x_residuals, iter,
        max_rotation, Tensor<double>(), residuals);
    ...
    if (r_params.print_level() >= 1) {
      molresponse::end_timer(world, "make_density_old", "make_density_old",
                             iter_timing);
    }
    if (r_params.print_level() >= 1) {
      molresponse::start_timer(world);
    ...
    if (r_params.print_level() >= 1) {
      molresponse::end_timer(world, "make_density_new", "make_density_new",
                             iter_timing);
    }
...
auto ExcitedResponse::update_response(
  ...
  for (size_t pass = 0; pass < 2; ++pass) {
    if (compute_y) {
      ...
      // TDA: standard Gram-Schmidt + Euclidean normalize on X only.
  ...
  // X_space theta_X = compute_theta_X(world, rotated_chi, xc,
  // r_params.calc_type());
  if (r_params.print_level() >= 1) {
    molresponse::start_timer(world);
  }
  X_space rotated_EOX(world, rotated_chi.num_states(),
                      rotated_chi.num_orbitals());
  if (r_params.localize() != "canon") {
...
X_space ExcitedResponse::create_virtual_ao_guess(World &world) const {
  ...
  auto F = [&](vector_real_function_3d x) {
    ...
    std::transform(x.begin(), x.end(), hf_exchange_X.begin(), [&](auto &xi) {
      ...
      std::accumulate(
          phi_0.begin(), phi_0.end(), f, [&](auto phi0, auto total) {
  ...
  auto first_positive =
      std::find_if(e_a.ptr(), e_a.ptr() + e_a.size(), is_positive);
...
X_space ExcitedResponse::create_response_guess(World &world) const {
  ...
  madness::print("norm of ao basis after projection: ",
                 norm2s_T(world, ao_vec));
  print("number of ao_basis_functions: ", ao_vec.size());
  auto overlap_S = matrix_inner(world, ao_vec, ao_vec);
  print("Overlap S : \n", overlap_S);
  Tensor<double> U, sigma, VT;
  ...
  auto two_int = [&](World &world, const vector_real_function_3d &phi0,
    ...
    for (int ii = 0; ii < phi0.size(); ii++) {
      for (int jj = 0; jj < phi0.size(); jj++) {
        ...
        // ii_ab = ii_ab.reshape(phi_a.size(), phi_a.size());
        //  I think there is a bug in tensor where reshape doesn't automatically
        //  change dim
        //
        print("after add\n", A);
      }

...
## Current Subworld Execution (March 2026)

Runtime behavior in the current code path (`MolresponseLib.hpp` + `FrequencyLoop.cpp`):

1. Subgroup ownership and protocol mode are planned once, then each subgroup executes only its owned channels/points.
2. Protocol-0 can run in channel-series mode; later protocols may switch to channel-point mode based on runtime policy.
3. Subgroups no longer perform a per-protocol global fence. A subgroup with available work can continue even if another subgroup has stalled points.
4. Guess seeding order for each point is:
   - same `(state, protocol, frequency)` archive if present,
   - coarser protocol archive for same `(state, frequency)`,
   - previous frequency (in-memory/archive) for the same state when applicable,
   - fresh initializer guess.
5. The solver does not auto-retry failed points from the "closest converged frequency" anymore. Failed points are recorded as unconverged and execution continues.
6. For every solved point, metadata records:
   - `saved` and `converged`,
   - `timings` (`wall_seconds`, `cpu_seconds`),
   - `restart_provenance`,
   - `solver_diagnostics` (`iterations_performed`, `final_residual_norm`, `final_density_change`, `used_fallback_retry`).

### Failure/Progress Behavior

1. Unconverged points are recorded and left as unconverged; they are not auto-retried from a "closest converged frequency" seed.
2. A single unconverged linear point no longer hard-aborts Stage 2 finalization.
3. Property assembly is partial-result capable:
   - `alpha`: skip frequencies missing required final-protocol direction states.
   - `beta`/`raman` component tasks: skip tasks that cannot load required states or throw during assembly.

### Beta Frequency Triplet Controls

Hyperpolarizability frequency-pair selection is now controlled by three response parameters:

1. `beta.shg` (default `true`)
   - Computes SHG-style triplets: `omegaB = omegaC`, `omegaA = -(omegaB + omegaC)`.
2. `beta.or` (default `false`)
   - Computes optical-rectification triplets: `omegaB = 0`, `omegaC` from input list, `omegaA = -omegaC`.
3. `beta.all_triplets` (default `false`)
   - Computes full `(omegaB, omegaC)` cartesian grid, with `omegaA = -(omegaB + omegaC)`.

...
GroundStateData::GroundStateData(World &world, std::string archiveFile, Molecule mol)
  ...
  if (world.rank() == 0) {
    print("GROUND_STATE_CONSTRUCT source=archive");
    molecule.print();
  }
  load(world);
}
...
GroundStateData::GroundStateData(World &world, std::shared_ptr<SCF> scf_calc)
  ...
  if (world.rank() == 0) {
    print("GROUND_STATE_CONSTRUCT source=scf");
    molecule.print();
  }
...
void GroundStateData::load(World &world) {
  ...
  if (k < 1 || k > 30) {
    if (world.rank() == 0)
      madness::print(
          "WARN INVALID_WAVELET_ORDER archive_k=", k, " fallback_k=8");
    k = 8;
  }
...
void GroundStateData::print_info() const {
  madness::print("GROUND_STATE_INFO");
  madness::print("  xc = ", xc);
  madness::print("  localization_method = ", localize_method);
  madness::print("  spin_restricted = ", spinrestricted);
  madness::print("  num_orbitals = ", num_orbitals);
  madness::print("  box_l = ", L);
  madness::print("  k = ", k);
  madness::print("  converged_threshold = ", converged_for_thresh);
  madness::print("  orbital_energies = ", energies);
  madness::print("------------------------------------------------------------");
}
...
void GroundStateData::prepareOrbitals(World &world, int current_k, double thresh) {
  ...
  if (original_k < current_k) {
    if (world.rank() == 0) {
      print("ERROR ORBITAL_PROJECT_FAILED reason=insufficient_k archive_k=", k,
            " requested_k=", current_k);
    }
  ...
  if (k != current_k || converged_for_thresh > thresh) {
    if (world.rank() == 0) {
      print("ORBITAL_RELOAD_REQUIRED");
      print("  current_k = ", k);
      print("  requested_k = ", current_k);
      print("  current_thresh = ", converged_for_thresh);
      print("  requested_thresh = ", thresh);
      print("------------------------------------------------------------");
    }
    needs_reload = true;
  }
...
Tensor<double> GroundStateData::tryLoadHamiltonianFromJson(World &world, const json &fock_json, double thresh, int k) {
  ...
  if (fock_json.find(protocol) == fock_json.end()) {
    std::cerr << "WARN FOCK_PROTOCOL_NOT_FOUND protocol=" << protocol
              << std::endl;
    return {};
  } else {

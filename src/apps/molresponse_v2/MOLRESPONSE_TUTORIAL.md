...
`MolresponseLib::response_calculation()` in `src/madness/chem/MolresponseLib.hpp` runs in three major stages:
...
Both call `computeFrequencyLoop(...)` (`src/apps/molresponse_v2/FrequencyLoop.cpp`) for each state or point lane.

Inside `computeFrequencyLoop(...)`:

- restart/load logic chooses best available initial vector:
  - exact point file,
  - coarser protocol point,
  - previous frequency continuation,
  - fresh initializer.
- the iterative solve is run through `solve_response_vector(...)`.
- results are saved and status is recorded in metadata.
- per-point timing is now recorded (`wall_seconds`, `cpu_seconds`) via `StateSolvePersistence::record_timing(...)`.

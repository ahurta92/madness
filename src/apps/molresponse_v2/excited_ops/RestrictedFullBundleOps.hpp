...
template <>
struct BundleModel<DynamicRestrictedResponse> {
    using response_type = DynamicRestrictedResponse;
...
private:
    static void compute_state_response_potentials(
        ...
        const auto &y_state = response_y(state);
        ...
        auto yphi  = mul(world, y_state, phi0, true);
        ...
        auto gamma_x =
            2.0 * jphi
            - c_xc * (K(world, phi0, x_state)(phi0) + K(world, y_state, phi0)(phi0));
        auto gamma_y =
            2.0 * jphi
            - c_xc * (K(world, phi0, y_state)(phi0) + K(world, x_state, phi0)(phi0));
        auto gamma_flat        = gs.Qhat(gamma_x);
        auto gamma_y_projected = gs.Qhat(gamma_y);
        gamma_flat.insert(gamma_flat.end(),
                          gamma_y_projected.begin(),
                          gamma_y_projected.end());

        auto v0_all      = gs.V_local * response_all(state) - c_xc * k0_flat;
    ...
    make_excited_bsh_operators(madness::World &world,
        ...
        ops.reserve(2 * n);
        orbital_shifts_out.reserve(2 * n);

        auto append_block = [&](double freq, bool apply_shift) {
            for (size_t p = 0; p < n; ++p) {
                const double energy = gs.getEnergies()(long(p));
                double shift = 0.0;
                if (apply_shift && (energy + freq) > 0.0)
                    shift = -(energy + freq + shift_factor);
                const double denom      = energy + freq + shift;
...
iterate_trial(madness::World &world,
              ResponseBundle<DynamicRestrictedResponse> &bundle,
              std::vector<double> &omega,
              const GroundStateData &gs,
              const ExcitedSolverParams &params) {
    return BundleModel<DynamicRestrictedResponse>::iterate_trial(

...
X_space TDDFT::compute_V0X(World& world,
  ...
  if (r_params.print_level() >= 10) {  // LEGACY_PATCH: collective first, print on rank 0
    auto xk0x = inner(Chi_copy, K0);  // all ranks must participate (collective)
    if (world.rank() == 0) { print("inner <X|K0|X>"); print(xk0x); }
  }

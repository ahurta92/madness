...
struct ResponseParameters : public QCCalculationParametersBase {
    // LEGACY_PATCH: QCCalculationParametersBase gained a pure-virtual get_tag() in the
    // current MADNESS API. Provide the concrete implementation to avoid abstract-type errors.
...
public:
    // LEGACY_PATCH: 'read(world, filename, tag)' was made private and renamed to
    // 'read_input' in the current MADNESS API.  Provide a public wrapper that
    // replicates the old behaviour (rank-0 reads, broadcasts *this).
    void read(World& world, const std::string& filename, const std::string& tag) {
    ...
    bool plot() const {
        return get<bool>("plot");
    }
    bool plot_range() const {
        return get<bool>("plot_range");
    }
    ...
    std::string core_type() const {
        return get<std::string>("core_type");
    }

    double plot_l() const {
        return get<double>("plot_l");
    }
    size_t plot_pts() const {
        return get<size_t>("plot_pts");
    }
    ...
    bool dconv_set() const {
        return get<bool>("dconv_set");
    }
    bool guess_xyz() const {
        return get<bool>("guess_xyz");
    }
    double lo() const {
        return get<double>("lo");
    }
    ...
    bool random() const {
        return get<bool>("random");
    }
    bool store_potential() const {
        return get<bool>("store_potential");
    }
    bool e_range() const {
        return get<bool>("e_range");
    }

    double e_range_lo() const {
        return get<double>("e_range_lo");
    }
    double e_range_hi() const {
        return get<double>("e_range_hi");
    }

    bool plot_initial() const {
        return get<bool>("plot_initial");
    }
    bool restart() const {
        return get<bool>("restart");
    }
    ...
    int vnucextra() const {
        return get<int>("vnucextra");
    }
    int loadbalparts() const {
        return get<int>("loadbalparts");
    }
    ...
    bool order2() const {
        return get<bool>("order2");
    }
    bool order3() const {
        return get<bool>("order3");
    }
    std::string d2_types() const {
        return get<std::string>("d2_types");
    }
    double omega() const {
        return get<double>("omega");
    }
    double L() const {
        return get<double>("l");
    }
    ...
    void read_and_set_derived_values(World& world, std::string inputfile, std::string tag) {
        // read the parameters from file and broadcast by tag
        ...
        if (world.rank() == 0)
            g_params.print_params(); // LEGACY_PATCH: rank gate
        // Ground state params
    ...
    // convenience getters
    double econv() const {
        return get<double>("econv");
    }
    bool first_run() const {
        return get<bool>("first_run");
    }
    std::string local() const {
        return get<std::string>("local");
    }
}; // namespace madness
} // namespace madness

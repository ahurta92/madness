...
void TDDFT::orbital_load_balance(World& world,
  ...
  if (world.size() > 1) {
    molresponse::start_timer(world);
    LoadBalanceDeux<3> lb(world);
    // LEGACY_PATCH: psi0 has n (num_orbitals) elements, NOT m (n_states).
    // Original code had lb.add_tree(psi0[i]) inside the i<m loop which caused
    // an out-of-bounds access whenever m != n (e.g. H2: m=4, n=1).
    // Fix: iterate ground orbitals separately over j<n.
    for (unsigned int j = 0; j < n; ++j) {
      lb.add_tree(psi0[j], lbcost<double, 3>(1.0, 8.0), false);
    }
    for (unsigned int i = 0; i < m; ++i) {
      for (unsigned int j = 0; j < n; ++j) {
        // add a tree for response orbitals
        lb.add_tree(Chi.X[i][j], lbcost<double, 3>(1.0, 8.0), false);
        lb.add_tree(Chi.Y[i][j], lbcost<double, 3>(1.0, 8.0), false);
      }
    }
...
X_space TDDFT::create_trial_functions(World& world,
                                      ...
                                      size_t print_level) {
  // Get size
  if (world.rank() == 0) print("In create trial functions");  // LEGACY_PATCH: rank gate
...
void TDDFT::PrintResponseVectorNorms(World& world,
                                     response_space f,
                                     std::string fname) {
  // LEGACY_PATCH: norm2() is collective — all ranks must call it; only rank 0 prints
  auto norms = f.norm2();
  if (world.rank() == 0) {

...
// LEGACY_PATCH: 'using namespace madness' is injected via -include legacy_preamble.h
// (see CMakeLists.txt). No explicit using-directive needed here.
...
class GroundParameters {
 ...
 public:
  ...
  void read(World& world, const std::string& filename) {
    ...
    // LEGACY_PATCH: updated read order to match current moldft archive format (version 4).
    // Old format: version, energy, spinrestricted, num_orbitals, energies, occ, aset, L, k, mol, xc
    // New format: version, energy, spinrestricted, L, k, mol, xc, localize_method,
    //             converged_for_thresh, num_orbitals, energies, occ, aset, [orbitals]
    // See SCF.cc line ~293 for the write side.
    unsigned int dummyversion;
    double dummy_energy;
    std::string dummy_localize;
    double dummy_thresh;
    std::vector<int> dummy2;

    archive::ParallelInputArchive input(world, filename.c_str());
    input& dummyversion;       // unsigned int  archive version (=4)
    input& dummy_energy;       // double        SCF total energy
    ...
    input& dummy_localize;     // std::string   localize method (new in v4)
    input& dummy_thresh;       // double        converged_for_thresh (new in v4)
    input& num_orbitals;       // unsigned int  number of alpha MOs
    input& energies;           // Tensor<double> orbital energies
    input& occ;                // Tensor<double> orbital occupations
    input& dummy2;             // std::vector<int> orbital set indices

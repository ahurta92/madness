...
density_vector read_and_create_density(World& world,
  ...
  // LEGACY_PATCH: removed if (world.rank()==0) guard — both functions handle
  // rank coordination internally (read() broadcasts r_params; ParallelInputArchive
  // is collective). With the guard, ranks != 0 got default-constructed r_params
  // and crashed in set_density_type with "what is this????".

...
void TDDFT::iterate_excited(World& world, X_space& Chi) {
  ...
  for (iter = 0; iter < r_params.maxiter(); ++iter) {
    ...
    if (r_params.print_level() >= 1) {
      // LEGACY_PATCH: norm2() is collective — compute on all ranks, print on rank 0
      auto xnorms = Chi.X.norm2();
      auto ynorms = Chi.Y.norm2();
      if (world.rank() == 0) {
  ...
  // Print final things
  // LEGACY_PATCH: norm2() is collective — compute on all ranks, print on rank 0
  auto final_x_norms = Chi.X.norm2();
  auto final_y_norms = Chi.Y.norm2();
  if (world.rank() == 0) {
